...
// 场景: AIJCASEE_ac_N_v010_dx5_2000
// 生成时间: 2026-06-02 15:49:33
// 风廓线: PowerLaw
// 风速: 3.93 m/s @ z_ref=15.9m
...
// ---- 物理参数 ----
// 网格间距    dx = 5.000 m
// 域尺寸      600.0 m x 800.0 m x 240.0 m
// 参考风速    3.93 m/s @ z_ref=15.9m
// 风廓线      PowerLaw
// 粗糙度长度  z0 = 0.300 m
// 幂律指数    alpha = 0.22
// 公式        U(z) = 3.93 * (z/15.9)^0.22
...
void main_setup() {
    ...
    parallel_for(lbm.get_N(), [&](ulong n) {
        ...
        // 入口/出口边界（Y 方向主导风，PowerLaw 风廓线）
        if(y == Ny-1u) {  // 入口：按风廓线设置速度
            lbm.flags[n] = TYPE_E;
            float3 u_in = windProfile(z);
            lbm.u.x[n] = u_in.x; lbm.u.y[n] = u_in.y; lbm.u.z[n] = u_in.z;
            return;
        }
        if(y == 0u)  { lbm.flags[n] = TYPE_E; return; }  // 出口：自由出流
        // X 方向侧面：自由滑移
    ...
    // DomainOrigin: (-300.00, -500.00, 0.00) m
    // Dx: 5.0000 m/格子
    float3 stl_offset = float3(60.0000f, 100.0000f, 0.0000f);  // -DomainOrigin/Dx
