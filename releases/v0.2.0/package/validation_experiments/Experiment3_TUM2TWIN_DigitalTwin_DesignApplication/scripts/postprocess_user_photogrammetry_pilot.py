...
def main():
    ...
    rows = []
    fig, axes = plt.subplots(2, len(Z_LEVELS), figsize=(20, 8), dpi=160, constrained_layout=True)
    for col, z in enumerate(Z_LEVELS):
        ...
        rows.append({
            ...
            "geometry_evidence_boundary": "photogrammetry STL exploratory pilot; not final watertight collision",
            "dx_m": DX,
            "z_index": z,
        ...
        ax1.contour(solid.astype(float), levels=[0.5], origin="lower", extent=extent, colors="black", linewidths=0.12)
        ax1.set_title(f"VR z~{z*DX:.0f} m")
        ax1.set_aspect("equal")
        ax1.set_xticks([])
        ax1.set_yticks([])
    cbar = fig.colorbar(im, ax=axes[1, :].tolist(), shrink=0.9)
    cbar.set_label("|U| / Uref")
    fig.suptitle("User-provided TUM Downtown photogrammetry STL FluidX3D exploratory audit, WD000, dx=2 m, 2000 steps")
