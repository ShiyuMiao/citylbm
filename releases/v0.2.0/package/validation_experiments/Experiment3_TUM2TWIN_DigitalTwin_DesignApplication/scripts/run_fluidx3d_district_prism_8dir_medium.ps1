...
foreach ($deg in $directions) {
    $label = "district_prism_wd{0:D3}_medium4m_10k" -f $deg
    ...
    if ($status -ne "ok") {
        throw "FluidX3D district prism medium run failed for $label. See $log"
    }
}

$summaryPath = Join-Path $caseRoot "figures\fluidx3d_district_prism_8dir_medium4m_10k_run_summary.csv"
