...
$srcStl = Join-Path $projectRoot "cfd_ready\core_prism_s2_network_porosity_collision_z0.stl"
$dstName = "core_prism_s2_network_porosity_collision_z0.stl"
...
foreach ($deg in $directions) {
    $label = "core_prism_s2_network_avg_wd{0:D3}_dx2m_spin6k_s3" -f $deg
    ...
    if ($status -ne "ok") {
        throw "FluidX3D S2 network-porosity run failed for $label. See $log"
    }
}

$summaryPath = Join-Path $caseRoot "figures\fluidx3d_core_prism_s2_network_porosity_8dir_dx2m_run_summary.csv"
