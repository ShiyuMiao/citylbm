...
def main():
    ...
    for deg in DIRECTIONS:
        label = f"district_prism_wd{deg:03d}_medium4m_10k"
    ...
    for z in Z_METRIC:
        ...
        summary = {
            "case": f"district_prism_equal_weighted_8dir_medium4m_10k_z{z}",
    ...
    csv_path = CASE_FIG_DIR / "fluidx3d_district_prism_8dir_medium4m_10k_metrics.csv"
    ...
    for ax, (deg, masked_vr, solid) in zip(axes.ravel(), audit_maps):
        ...
        ax.contour(solid.astype(float), levels=[0.5], origin="lower", extent=extent, colors="black", linewidths=0.11)
    ...
    fig.suptitle("FluidX3D TUM2TWIN district prism 8-direction medium audit, z approx 8 m, dx=4 m, 10000 steps")
    panel_path = CASE_FIG_DIR / "fluidx3d_district_prism_8dir_medium4m_10k_vr_panel_z8m.png"
    ...
    ax.contour(solid_ref.astype(float), levels=[0.5], origin="lower", extent=extent, colors="black", linewidths=0.12)
    ax.set_title("District prism equal-weighted 8-direction VR mean, z approx 8 m, dx=4 m")
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    ax.set_aspect("equal")
    fig.colorbar(im, ax=ax, label="mean |U| / Uref")
    mean_path = CASE_FIG_DIR / "fluidx3d_district_prism_8dir_medium4m_10k_equal_weighted_vr_z8m.png"
