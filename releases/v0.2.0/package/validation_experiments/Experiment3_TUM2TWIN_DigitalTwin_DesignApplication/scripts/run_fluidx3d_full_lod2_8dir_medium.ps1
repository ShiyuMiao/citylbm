...
foreach ($deg in $directions) {
    $label = "full_lod2_wd{0:D3}_medium2m_10k" -f $deg
    ...
    if ($status -ne "ok") {
        throw "FluidX3D medium matrix failed for $label. See $log"
    }
}

$summaryPath = Join-Path $caseRoot "figures\fluidx3d_full_lod2_8dir_medium2m_10k_run_summary.csv"
