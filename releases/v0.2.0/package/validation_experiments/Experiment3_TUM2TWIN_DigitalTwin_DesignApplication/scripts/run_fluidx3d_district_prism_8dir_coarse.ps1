...
$env:TUM2TWIN_STL = "district_prism_collision_z0.stl"
$env:TUM2TWIN_NX = "373"
$env:TUM2TWIN_NY = "346"
$env:TUM2TWIN_NZ = "49"
$env:TUM2TWIN_DX = "6.0"
...
foreach ($deg in $directions) {
    $label = "district_prism_wd{0:D3}_coarse6m_10k" -f $deg
    ...
    if ((Test-Path $finalU) -and (Test-Path $finalFlags)) {
        $summary += [pscustomobject]@{
            label = $label; wind_deg = $deg; status = "skipped_existing"; elapsed_s = 0
            log = $log; final_u = $finalU; final_flags = $finalFlags
        }
    ...
    if ($status -ne "ok") {
        throw "FluidX3D district prism run failed for $label. See $log"
    }
}

$summaryPath = Join-Path $caseRoot "figures\fluidx3d_district_prism_8dir_coarse6m_10k_run_summary.csv"
