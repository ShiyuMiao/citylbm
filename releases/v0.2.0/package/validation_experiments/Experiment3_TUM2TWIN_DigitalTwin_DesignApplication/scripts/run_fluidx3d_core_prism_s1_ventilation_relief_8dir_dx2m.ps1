...
$srcStl = Join-Path $projectRoot "cfd_ready\core_prism_s1_ventilation_relief_collision_z0.stl"
$dstName = "core_prism_s1_ventilation_relief_collision_z0.stl"
...
foreach ($deg in $directions) {
    $label = "core_prism_s1_relief_avg_wd{0:D3}_dx2m_spin6k_s3" -f $deg
    ...
    if ($status -ne "ok") {
        throw "FluidX3D S1 ventilation-relief run failed for $label. See $log"
    }
}

$summaryPath = Join-Path $caseRoot "figures\fluidx3d_core_prism_s1_ventilation_relief_8dir_dx2m_run_summary.csv"
