...
def main():
    ...
    for wd in WIND_DIRS:
        ...
        for sample_idx, step in SAMPLES:
            ...
            for z in Z_LEVELS:
                ...
                sample_row.update(stat_values(vals))
    ...
    for ax, (wd, masked_vr) in zip(axes.ravel(), panel_data):
        ...
        ax.set_title(f"WD {wd:03d}, time-mean, z~{PANEL_Z*DX:.0f} m")
        ax.set_aspect("equal")
        ax.set_xticks([])
        ax.set_yticks([])
    cbar = fig.colorbar(im, ax=axes.ravel().tolist(), shrink=0.9)
    cbar.set_label("VR = |U| / Uref")
    fig.suptitle("Core prism FluidX3D, time-mean of 3 samples, 8 directions, dx=2 m, z~2 m")
    panel_png = CASE_FIG_DIR / "fluidx3d_core_prism_timesampled_8dir_dx2m_vr_panel_z2m.png"
    ...
    ax.set_title("Time-mean + equal-weighted 8-direction VR, core prism, z~2 m")
    ax.set_aspect("equal")
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    fig.colorbar(im, ax=ax, label="VR = |U| / Uref")
    equal_png = CASE_FIG_DIR / "fluidx3d_core_prism_timesampled_8dir_dx2m_equal_weighted_vr_z2m.png"
    fig.savefig(equal_png)
    plt.close(fig)

    metrics_csv = CASE_FIG_DIR / "fluidx3d_core_prism_timesampled_8dir_dx2m_metrics.csv"
