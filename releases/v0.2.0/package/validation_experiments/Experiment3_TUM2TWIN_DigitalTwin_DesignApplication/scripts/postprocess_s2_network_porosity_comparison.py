...
def compute_scenario(scenario_id: str):
    label_template = LABEL_TEMPLATES[scenario_id]
    rows = []
    panel_data = []
    ...
    return {"scenario": scenario_id, "meta": panel_meta, "rows": rows, "panel_data": panel_data, "panel_solid": panel_solid, "equal_maps": equal_maps}
...
def plot_s2_panel(s2):
    extent = extent_xy(s2["meta"])
    fig, axes = plt.subplots(2, 4, figsize=(18, 9), dpi=170, constrained_layout=True)
    for ax, (wd, masked_vr) in zip(axes.ravel(), s2["panel_data"]):
        im = ax.imshow(masked_vr, origin="lower", extent=extent, cmap="turbo", vmin=0.0, vmax=1.6, interpolation="nearest")
        ax.contour(s2["panel_solid"].astype(float), levels=[0.5], origin="lower", extent=extent, colors="black", linewidths=0.12)
        ax.set_title(f"S2 WD {wd:03d}, z~{PANEL_Z*DX:.0f} m")
        ax.set_aspect("equal")
        ax.set_xticks([])
        ax.set_yticks([])
    cbar = fig.colorbar(im, ax=axes.ravel().tolist(), shrink=0.9)
    cbar.set_label("VR = |U| / Uref")
    fig.suptitle("S2 network-porosity FluidX3D, time-mean of 3 samples, 8 directions, dx=2 m")
    path = CASE_FIG_DIR / "fluidx3d_core_prism_s2_network_porosity_8dir_dx2m_vr_panel_z2m.png"
    fig.savefig(path)
    plt.close(fig)
    return path


def comparison_rows(reference, target, label):
    ref_equal = {int(r["z_index"]): r for r in reference["rows"] if r["wind_deg"] == "equal_weighted"}
    tgt_equal = {int(r["z_index"]): r for r in target["rows"] if r["wind_deg"] == "equal_weighted"}
    metric_names = ["vr_mean", "vr_p75", "vr_p90", "vr_p95", "vr_max", "stagnation_ratio_vr_lt_0p2", "accelerated_ratio_vr_gt_0p6", "high_ratio_vr_gt_1p0"]
    rows = []
    for z in Z_LEVELS:
        row = {
            "evidence_type": "newly_run",
            ...
            "z_index": z,
            "z_height_m_approx": z * DX,
            "reference_open_cells": ref_equal[z]["open_cells"],
            "target_open_cells": tgt_equal[z]["open_cells"],
            "delta_open_cells": int(tgt_equal[z]["open_cells"]) - int(ref_equal[z]["open_cells"]),
        }
        for m in metric_names:
            rv = float(ref_equal[z][m])
            tv = float(tgt_equal[z][m])
            row[f"reference_{m}"] = rv
            row[f"target_{m}"] = tv
            row[f"delta_{m}"] = tv - rv
            row[f"relative_delta_{m}"] = (tv - rv) / rv if rv != 0.0 else ""
        rows.append(row)
    return rows


def open_cell_delta_rows(reference, target, label):
    rows = []
    for z in Z_LEVELS:
        ref_map = reference["equal_maps"][z]
        tgt_map = target["equal_maps"][z]
        ref_open = ~np.isnan(ref_map)
        tgt_open = ~np.isnan(tgt_map)
        common = ref_open & tgt_open
        newly_open = ~ref_open & tgt_open
        closed = ref_open & ~tgt_open
        delta = tgt_map - ref_map
        common_delta = delta[common]
        new_vals = tgt_map[newly_open]
        row = {
            "evidence_type": "newly_run",
            "comparison": label,
            "averaging": "time_mean_3_samples_then_direction_mean",
        ...
        if common_delta.size:
            ...
            row.update({k: "" for k in ["common_delta_vr_mean", "common_delta_vr_p05", "common_delta_vr_p50", "common_delta_vr_p95", "common_delta_vr_min", "common_delta_vr_max", "common_ratio_delta_gt_0p02", "common_ratio_delta_lt_minus_0p02"]})
        if new_vals.size:
            row.update({
                "newly_open_target_vr_mean": float(new_vals.mean()),
                "newly_open_target_vr_p95": float(np.percentile(new_vals, 95)),
            ...
            row.update({k: "" for k in ["newly_open_target_vr_mean", "newly_open_target_vr_p95", "newly_open_stagnation_ratio_vr_lt_0p2", "newly_open_acceleration_ratio_vr_gt_0p6"]})
        rows.append(row)
    return rows


def plot_three_scenario_summary(s0, s1, s2):
    heights = [z * DX for z in Z_LEVELS]
    scenarios = [s0, s1, s2]
    names = ["S0", "S1", "S2"]
    means = []
    stags = []
    for scenario in scenarios:
        equal = {int(r["z_index"]): r for r in scenario["rows"] if r["wind_deg"] == "equal_weighted"}
        means.append([float(equal[z]["vr_mean"]) for z in Z_LEVELS])
        stags.append([float(equal[z]["stagnation_ratio_vr_lt_0p2"]) for z in Z_LEVELS])
    x = np.arange(len(heights))
    width = 0.25
    fig, axes = plt.subplots(1, 2, figsize=(13, 5), dpi=170, constrained_layout=True)
    for idx, name in enumerate(names):
        axes[0].bar(x + (idx - 1) * width, means[idx], width, label=name)
        axes[1].bar(x + (idx - 1) * width, stags[idx], width, label=name)
    for ax in axes:
        ax.set_xticks(x, [f"{h:g} m" for h in heights])
        ax.legend()
    ...
    path = CASE_FIG_DIR / "fluidx3d_s0_s1_s2_design_sensitivity_height_metric_comparison.png"
    fig.savefig(path)
    plt.close(fig)
    return path


def plot_delta_panel(s0, s2):
    extent = extent_xy(s0["meta"])
    z = PANEL_Z
    s0_map = s0["equal_maps"][z]
    s2_map = s2["equal_maps"][z]
    common = ~np.isnan(s0_map) & ~np.isnan(s2_map)
    delta = np.where(common, s2_map - s0_map, np.nan)
    ...
    axes[1].imshow(np.ma.masked_invalid(s2_map), origin="lower", extent=extent, cmap="turbo", vmin=0.0, vmax=1.6, interpolation="nearest")
    axes[1].set_title("S2 equal-weighted VR, z~2 m")
    ...
    axes[2].set_title("S2 - S0 VR delta, common open cells")
    for ax in axes:
        ax.set_aspect("equal")
        ax.set_xlabel("x (m)")
        ax.set_ylabel("y (m)")
    ...
    path = CASE_FIG_DIR / "fluidx3d_s0_s2_network_porosity_equal_weighted_vr_delta_z2m.png"
    fig.savefig(path)
    plt.close(fig)
    return path
...
def main():
    s0 = compute_scenario("S0")
    s1 = compute_scenario("S1")
    s2 = compute_scenario("S2")

    metrics_path = CASE_FIG_DIR / "fluidx3d_core_prism_s2_network_porosity_8dir_dx2m_metrics.csv"
    write_csv(metrics_path, s2["rows"])
    s2_s0_comp = comparison_rows(s0, s2, "S2_minus_S0")
    s2_s0_comp_path = CASE_FIG_DIR / "fluidx3d_s0_s2_network_porosity_metric_comparison.csv"
    write_csv(s2_s0_comp_path, s2_s0_comp)
    s2_s1_comp = comparison_rows(s1, s2, "S2_minus_S1")
    s2_s1_comp_path = CASE_FIG_DIR / "fluidx3d_s1_s2_network_porosity_metric_comparison.csv"
    write_csv(s2_s1_comp_path, s2_s1_comp)
    s2_open = open_cell_delta_rows(s0, s2, "S2_minus_S0_common_and_newly_open_cells")
    s2_open_path = CASE_FIG_DIR / "fluidx3d_s0_s2_network_porosity_common_open_delta_summary.csv"
    write_csv(s2_open_path, s2_open)

    panel = plot_s2_panel(s2)
    delta = plot_delta_panel(s0, s2)
    height_fig = plot_three_scenario_summary(s0, s1, s2)

    for path in [metrics_path, s2_s0_comp_path, s2_s1_comp_path, s2_open_path, panel, delta, height_fig]:
