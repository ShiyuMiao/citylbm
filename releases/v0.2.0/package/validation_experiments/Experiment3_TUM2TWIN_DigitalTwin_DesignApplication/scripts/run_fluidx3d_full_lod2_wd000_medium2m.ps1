...
} | Export-Csv -NoTypeInformation -Encoding UTF8 -Path (Join-Path $caseRoot "figures\fluidx3d_full_lod2_wd000_medium2m_10k_run_summary.csv")

Write-Host ("{0}: {1}, {2}s" -f $label, $status, [math]::Round(($end - $start).TotalSeconds, 2))
if ($status -ne "ok") {
    throw "FluidX3D medium run failed. See $log"
}
