...
FIGURE_CAPTIONS = [
    {
        ...
        "caption_zh": "图 E3-1. TUM Downtown 核心校园街区行人高度 FluidX3D/VTK 风速比筛查图。结果来自 dx=2 m、八个来流方向、三个采样时刻的核心闭合棱柱碰撞几何，用于人工审核低风速区、方向一致性和建筑周边滞风格局。该图支持“行人层以低速和通风不足为主”的筛查性结论，但不支持年度舒适度合规、实测验证或污染物扩散结论。",
    ...
    },
    {
        ...
        "caption_zh": "图 E3-2. 基础建筑形态参数与 20-50 m 局地环境带风速响应的多变量稳健性分析。排序回归系数和置换重要度显示，在本样本中局地围合度、平均高度和综合围合指标比单体占地面积、平面伸长率或紧凑度更能解释风速差异。由于交叉验证解释力有限，该图应写成可解释筛查证据，而不是高精度预测模型。",
    ...
    },
    {
        ...
        "caption_zh": "图 E3-4. 0-20 m 近立面带到 20-50 m 局地环境带的风速恢复阈值规则筛查。分析在同一组 101 个保留建筑构件上比较近立面与局地环境响应，并提取样本内 tertile 组合规则。最佳简单规则提示较低相对竖向尺度和特定平面形态组合更易出现局地恢复，但该阈值仅用于数字孪生样本内设计筛查，不能外推为通用规范或实测验证结论。",
    ...
    },
    {
        ...
        "caption_zh": "图 E3-S2. 实验3八风向各向异性与设计扇区响应补充图。图中比较 S0 行人层 mean VR、行人层滞风比例、40 m-2 m 竖向恢复以及 S2 common-open-cell 局地响应。结果显示行人层低速和高滞风在八个来流方向中近似全向存在，而 S2 的局部响应具有方向性，最强响应出现在 315 deg；但 S1/S2 全局行人层 mean-VR delta 在全部方向上仍为负。该图支持风扇区耦合的设计解释，不支持年度风玫瑰合规或成功优化宣称。",
    ...
    },
    {
        ...
        "caption_zh": "图 E3-S4. 建筑形态阶段转换分析补充图。图中把 101 个保留建筑构件的风响应分解为 0-20 m 近立面低速阶段、20-50 m 局地环境恢复阶段和方向性响应阶段。结果显示近立面带几乎为低速饱和区，而 20-50 m 带揭示了与相对竖向尺度、平面延展性和局地围合相关的恢复差异。该图用于数字孪生设计筛查，不代表实测验证的因果阈值或通用设计规范。",
    ...
    },
    {
        ...
        "caption_zh": "图 E3-S5. 20-50 m 局地环境带的建筑形态方向性指纹分析。图中将 101 个保留建筑构件的八风向 mean VR 范围、方向响应比、最佳响应风向与基础形态参数和阶段转换类型联系起来。persistent shelter 构件同时具有低平均 VR 和低方向范围，而 near-to-context recovery 和 directionally reactive 构件更能表现出来流扇区响应的差异。该图支持数字孪生设计筛查，不支持实测验证的因果阈值或年度风玫瑰合规评价。",
