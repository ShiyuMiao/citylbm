...
def main():
    ...
    for deg in DIRECTIONS:
        label = f"full_lod2_wd{deg:03d}_coarse4m_10k"
    ...
    # Equal-weighted direction summary for every metric height.
    for z in Z_METRIC:
        layer_rows = [r for r in rows if int(r["z_index"]) == z]
        summary = {
            "case": f"full_lod2_equal_weighted_8dir_coarse4m_10k_z{z}",
        ...
        for key in [
            ...
            "accelerated_ratio_vr_gt_1p2",
        ]:
            summary[key] = float(np.mean([float(r[key]) for r in layer_rows]))
        rows.append(summary)

    csv_path = CASE_FIG_DIR / "fluidx3d_full_lod2_8dir_coarse4m_10k_metrics.csv"
    ...
    for ax, (deg, masked_vr, solid) in zip(axes.ravel(), audit_maps):
        ...
        ax.contour(solid.astype(float), levels=[0.5], origin="lower", extent=extent, colors="black", linewidths=0.22)
    ...
    fig.suptitle("FluidX3D TUM2TWIN full LoD2 8-direction coarse audit, z approx 8 m, dx=4 m, 10000 steps")
    panel_path = CASE_FIG_DIR / "fluidx3d_full_lod2_8dir_coarse4m_10k_vr_panel_z8m.png"
    ...
    ax.contour(solid_ref.astype(float), levels=[0.5], origin="lower", extent=extent, colors="black", linewidths=0.28)
    ax.set_title("Equal-weighted 8-direction VR mean, z approx 8 m, dx=4 m")
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    ax.set_aspect("equal")
    fig.colorbar(im, ax=ax, label="mean |U| / Uref")
    mean_path = CASE_FIG_DIR / "fluidx3d_full_lod2_8dir_coarse4m_10k_equal_weighted_vr_z8m.png"
