...
def build_paper_text_zh(df: pd.DataFrame, corr: pd.DataFrame, stage: pd.DataFrame, stats: dict[str, float]) -> str:
    ...
    recovery = stage.set_index("stage_transition_class").loc["near_to_context_recovery"]
    reactive = stage.set_index("stage_transition_class").loc["directionally_reactive"]
    return f"""# 建筑形态方向性风响应结论

evidence_type: newly_run + blocked

为了进一步把建筑形式与风环境的关系从“平均风速差异”推进到“来流方向响应差异”，本研究在 101 个保留建筑构件上计算了 20-50 m 局地环境带的方向性指纹，包括八个风向 mean VR 的最大-最小范围、方向响应比以及最佳/最弱响应风向。结果显示，20-50 m 带的平均方向范围为 `{range_mean:.6f}`，平均方向响应比为 `{reactivity_mean:.3f}`。方向范围与平均高度的 Spearman rho 为 `{corr_range.loc['mean_height_m','spearman_rho']:.3f}`，与 50 m 扇区围合度的 rho 为 `{corr_range.loc['sector_enclosure_ratio_r50m','spearman_rho']:.3f}`，与综合围合得分的 rho 为 `{corr_range.loc['relative_enclosure_score','spearman_rho']:.3f}`。这说明在该校园街区样本中，较强围合和竖向体量不仅会压低局地平均 VR，也会削弱不同来流方向能够带来的响应差异。

阶段类型之间的方向性差异更明显。persistent shelter 类的平均方向范围为 `{persistent['mean_directional_range_vr']:.6f}`，near-to-context recovery 类为 `{recovery['mean_directional_range_vr']:.6f}`，directionally reactive 类为 `{reactive['mean_directional_range_vr']:.6f}`，不同阶段类型之间 Kruskal-Wallis p 值为 `{stats['kruskal_range_p']:.4g}`。因此，本实验在传统“街谷围合导致近地遮蔽”认识之外提供的新认知是：有效的校园局地通风恢复不只体现为 20-50 m 平均 VR 提高，还体现为对不同来流扇区的响应能力增强。不具有这种方向性指纹的开放或孔隙，可能只是低速背景中的形式开放，这也解释了 S1/S2 为什么没有在全局行人层带来正向恢复。
