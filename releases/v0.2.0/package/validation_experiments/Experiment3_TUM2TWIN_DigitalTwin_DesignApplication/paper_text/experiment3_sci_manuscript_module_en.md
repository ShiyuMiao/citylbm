...
A further morphology-response typology clusters the 101 retained building components using only basic form parameters and then interprets each group with FluidX3D-derived wind metrics. The groups differ in 20-50 m recovery delta (Kruskal-Wallis p=0.0001682). The strongest recovery archetype, R1_A1_linear_low_relative_height_recovery, has mean 20-50 m VR of 0.0237 and recovery delta of 0.0080, whereas the weakest group, R4_A5_open_or_mixed_low_response, has mean 20-50 m VR of 0.0043 and recovery delta of 0.0007. This supports a design-screening conclusion that pedestrian-layer recovery is better framed as a combined response of relative vertical massing, elongation and local enclosure than as a single footprint, height or porosity effect.
