...
### 7.6 Morphology Threshold Design-Rule Addendum

The threshold design-rule layer compares the same 101 retained central building components between the facade-adjacent 0-20 m band and the 20-50 m local-context band. The mean near-facade VR is `0.0032`, while the mean local-context VR is `0.0056`, giving a mean recovery delta of `0.0024`. The strongest monotonic descriptor of this recovery delta is `height/sqrt(area)` with Spearman rho `-0.416`, and the best simple sample-internal rule is `mean_height_m=low_tertile + elongation_ratio=high_tertile`, with mean recovery delta `0.0057` and top-recovery share `0.857`.

This strengthens the paper's design conclusion in a careful way. It suggests that the 20-50 m local-context band is the useful diagnostic scale for morphology-sensitive wind recovery, while the 0-20 m facade-adjacent band is too uniformly sheltered to distinguish design mechanisms. The rule should be written as digital-twin screening evidence within this modeled campus case, not as a generalizable or field-validated design threshold.

### 7.7 SCI Manuscript Module Addendum
...
The submission-readiness layer adds bilingual captions for the four planned main figures, three supplementary figures, and three planned tables, plus an asset-level audit in `reports/experiment3_submission_readiness_audit.md` and `manifests/experiment3_submission_readiness_checklist.csv`. All ten assets exist and are ready for manual review. This layer improves paper usability because each figure/table is tied to its evidence type, source data, safe manuscript role, and boundary condition.
...
The effect-size layer recomputes uncertainty intervals from already archived simulation and morphology outputs. For S0, it uses 24 direction-sample units at each height; for vertical recovery, it uses paired z~40 m minus z~2 m differences; for S1/S2, it uses 8-direction z~2 m ranges; and for morphology, it uses 101 retained building components.
...
### 7.12 Building-Form Wind-Response Archetype Addendum

The morphology-response layer was deepened by clustering the 101 retained central building components using only basic form parameters: footprint area, mean height, height/sqrt(area), compactness, elongation, local built fraction within 30 m, sector enclosure within 50 m, and combined enclosure score. Wind variables were used only after clustering to interpret the groups. The resulting archetypes have statistically different 20-50 m recovery deltas, with Kruskal-Wallis `p=0.0001682`.

The strongest recovery group is `R1_A1_linear_low_relative_height_recovery`, with mean 20-50 m VR `0.0237` and mean near-to-context recovery delta `0.0080`. The weakest group is `R4_A5_open_or_mixed_low_response`, with mean 20-50 m VR `0.0043` and recovery delta `0.0007`. This gives the paper a clearer building-form conclusion: in this campus-core case, wind recovery is better explained as a combined morphology response involving relative vertical massing, elongation, and local enclosure than as a single effect of footprint area, height, or opening area. The claim remains a digital-twin/FluidX3D screening result and is not a field-validated causal typology.
