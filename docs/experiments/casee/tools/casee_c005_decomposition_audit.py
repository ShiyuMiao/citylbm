#!/usr/bin/env python3
"""Audit C005 dx=2 4x1x1 domain-decomposition Case E ablation."""
...
CASE_DIR_C005 = CASE_DIR / "native_cases" / "casee_native_dx2_yn_sgs_gshift1_zoff1_nu0p001_dom4x1x1_pmodes_steps48000_spin12000"
ZCENTER_BASELINE_CSV = RESULTS_DIR / "casee_native_dx2_zcenter_gshift1_nu001_pmodes_probe_time_mean.csv"
OUT_JSON = RESULTS_DIR / "casee_c005_decomposition_audit.json"
OUT_CSV = RESULTS_DIR / "casee_c005_decomposition_audit.csv"
OUT_MD = RESULTS_DIR / "casee_c005_decomposition_audit.md"
...
def manifest_protocol_ok(path: Path) -> bool:
    ...
    return (
        ...
        and data.get("origin_z_offset_m") == 1.0
        and data.get("domain_decomposition") == [4, 1, 1]
...
def main() -> int:
    candidate_csv = latest("casee_c005_dx2_decomp4x1x1_*_probe_time_mean.csv")
    run_log = latest("fluidx3d_c005_dx2_decomp4x1x1_run_*.log")
    err_log = latest("fluidx3d_c005_dx2_decomp4x1x1_run_*.err.log")
    compile_log = RESULTS_DIR / "fluidx3d_c005_dx2_decomp4x1x1_compile.log"
    compile_err = RESULTS_DIR / "fluidx3d_c005_dx2_decomp4x1x1_compile.err.log"
    manifest = CASE_DIR_C005 / "citylbm_native_case_manifest.json"
    missing = [
        name
        for name, path in {
    ...
    if missing:
        payload: Dict[str, Any] = {
            ...
            "claim_readiness": "blocked_c005_audit",
            "formal_release_allowed": False,
            "formal_accuracy_claim_supported": False,
            "boundary": "C005 has no auditable result until generated setup, compile log, run log, and 80-probe CSV exist.",
        }
    else:
        ...
        mae_consistent = abs(metric_delta["mae_pp"]) <= 1.0
        r2_consistent = abs(metric_delta["r2"]) <= 0.2
        decomposition_consistent = mae_consistent and r2_consistent and pearson_positive and protocol_ok
        improved_vs_baseline = (
            candidate_metrics["mae_pp"] < baseline_metrics["mae_pp"]
            and candidate_metrics["r2"] > baseline_metrics["r2"]
            and candidate_metrics["pearson"] > baseline_metrics["pearson"]
        )
        payload = {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "candidate_id": "C005_dx2_domain_decomposition_ablation",
            "status": "completed_decomposition_consistent" if decomposition_consistent else "completed_decomposition_sensitivity_warning",
            "evidence_type": "newly_run",
            "claim_readiness": "limitations_ready_runtime_decomposition_ablation; blocked formal accuracy release",
            "formal_release_allowed": False,
            "formal_accuracy_claim_supported": False,
            "pass_condition_met": decomposition_consistent,
            ...
            "mae_consistent_within_1pp": mae_consistent,
            "r2_consistent_within_0p2": r2_consistent,
            "improved_vs_zcenter_baseline": improved_vs_baseline,
            ...
            "zcenter_baseline_metrics": baseline_metrics,
            "candidate_metrics": candidate_metrics,
            "metric_delta_vs_zcenter_baseline": metric_delta,
            "consistency_thresholds": {
                "abs_mae_delta_pp_max": 1.0,
                "abs_r2_delta_max": 0.2,
                "pearson_must_remain_positive": True,
            },
            "boundary": (
                "C005 is a completed dx=2 4x1x1 domain-decomposition ablation under the official z=2 m "
                "raw_trilinear protocol. It is runtime/reproducibility evidence only and cannot support "
                "formal v0.4.0, mesh independence, or a default accuracy upgrade."
            ),
        }
    ...
    fields = [
        ...
        "mae_consistent_within_1pp",
        "r2_consistent_within_0p2",
        "pearson_positive",
        "pass_condition_met",
        ...
        writer.writerow(
            {
                ...
                "mae_consistent_within_1pp": payload.get("mae_consistent_within_1pp"),
                "r2_consistent_within_0p2": payload.get("r2_consistent_within_0p2"),
