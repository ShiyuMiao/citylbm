#!/usr/bin/env python3
"""Audit C003 dx=2 no-z-center Case E z-origin ablation."""
...
CASE_DIR_C003 = CASE_DIR / "native_cases" / "casee_native_dx2_yn_sgs_gshift1_zoff0_nu0p001_pmodes_steps48000_spin12000"
ZCENTER_BASELINE_CSV = RESULTS_DIR / "casee_native_dx2_zcenter_gshift1_nu001_pmodes_probe_time_mean.csv"
PREEXISTING_NO_ZCENTER_CSV = RESULTS_DIR / "casee_native_dx2_gshift1_nu001_pmodes_probe_time_mean.csv"
OUT_JSON = RESULTS_DIR / "casee_c003_zorigin_ablation_audit.json"
OUT_CSV = RESULTS_DIR / "casee_c003_zorigin_ablation_audit.csv"
OUT_MD = RESULTS_DIR / "casee_c003_zorigin_ablation_audit.md"
...
def main() -> int:
    candidate_csv = latest("casee_c003_dx2_no_zcenter_*_probe_time_mean.csv")
    run_log = latest("fluidx3d_c003_dx2_no_zcenter_run_*.log")
    err_log = latest("fluidx3d_c003_dx2_no_zcenter_run_*.err.log")
    compile_log = RESULTS_DIR / "fluidx3d_c003_dx2_no_zcenter_compile.log"
    compile_err = RESULTS_DIR / "fluidx3d_c003_dx2_no_zcenter_compile.err.log"
    missing = [
        name
        for name, path in {
            "candidate_csv": candidate_csv,
            "zcenter_baseline_csv": ZCENTER_BASELINE_CSV,
            "preexisting_no_zcenter_csv": PREEXISTING_NO_ZCENTER_CSV,
            "run_log": run_log,
            "compile_log": compile_log,
            "case_manifest": CASE_DIR_C003 / "citylbm_native_case_manifest.json",
        }.items()
        if path is None or not path.exists()
    ]
    ...
    if missing:
        payload: Dict[str, Any] = {
            ...
            "claim_readiness": "blocked_c003_audit",
            "formal_release_allowed": False,
            "formal_accuracy_claim_supported": False,
            "boundary": "C003 has no auditable result until generated setup, compile log, run log, and 80-probe CSV exist.",
        }
    else:
        assert candidate_csv is not None
        zcenter_metrics = metrics(ZCENTER_BASELINE_CSV)
        preexisting_metrics = metrics(PREEXISTING_NO_ZCENTER_CSV)
        ...
        consistent_with_preexisting = (
            abs(candidate_metrics["mae_pp"] - preexisting_metrics["mae_pp"]) < 2.0
            and abs(candidate_metrics["r2"] - preexisting_metrics["r2"]) < 0.5
        )
        payload = {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "candidate_id": "C003_dx2_no_zcenter_ablation",
            "status": "completed_ablation_supports_zorigin_sensitivity" if degraded_vs_zcenter else "completed_ablation_inconclusive",
            "evidence_type": "newly_run",
            "claim_readiness": "limitations_ready_zorigin_ablation; blocked formal accuracy release",
            "formal_release_allowed": False,
            "formal_accuracy_claim_supported": False,
            "pass_condition_met": degraded_vs_zcenter,
            "log_completed_48000": log_completed,
            "probe_count_ok": probe_count_ok,
            "consistent_with_preexisting_no_zcenter": consistent_with_preexisting,
            "zcenter_baseline_csv": file_status(ZCENTER_BASELINE_CSV),
            "preexisting_no_zcenter_csv": file_status(PREEXISTING_NO_ZCENTER_CSV),
            ...
            "case_manifest": file_status(CASE_DIR_C003 / "citylbm_native_case_manifest.json"),
            "zcenter_baseline_metrics": zcenter_metrics,
            "preexisting_no_zcenter_metrics": preexisting_metrics,
            "candidate_metrics": candidate_metrics,
            "metric_delta_vs_zcenter_baseline": delta(candidate_metrics, zcenter_metrics),
            "metric_delta_vs_preexisting_no_zcenter": delta(candidate_metrics, preexisting_metrics),
            "boundary": (
                "C003 is a completed official z=2 m raw_trilinear z-origin ablation. It supports treating "
                "z-center behavior as a protocol/near-wall sensitivity diagnostic, not as a validated default "
                "accuracy model. It does not support formal v0.4.0."
            ),
        }
    ...
    fields = [
        ...
        "consistent_with_preexisting_no_zcenter",
        "pass_condition_met",
        ...
        writer.writerow(
            {
                ...
                "consistent_with_preexisting_no_zcenter": payload.get("consistent_with_preexisting_no_zcenter"),
