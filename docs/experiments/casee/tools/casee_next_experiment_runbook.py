...
def command_rows(release_gate: Dict[str, Any], blockers: Dict[str, Any], dx1_readiness: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    rows: List[Dict[str, Any]] = [
        ...
        {
            "runbook_id": "R006_wall_model_followup",
            "stage": "native_case_generation_then_run",
            "enabled_now": not gpu_blocked,
            "evidence_type": "blocked_until_gpu_ready" if gpu_blocked else "newly_run_when_executed",
            "purpose": "Test the default-off voxel-dilation wall/ground follow-up aimed at near-wall official z=2 m errors.",
            "trigger_condition": "GPU ready and official preflight passes; wall follow-up codegen gate must remain pass/fail closed.",
            "command": "python docs/experiments/casee/tools/generate_native_casee.py --dx 2 --steps 48000 --spinup 12000 --sample-dt 2000 --ground-offset-cells 1 --origin-z-offset-m 1.0 --nu-lbm 0.001 --domain-x 4 --domain-y 1 --domain-z 1 --wall-model voxel_dilation --wall-dilation-cells 1 --no-subgrid",
        ...
        {
            "runbook_id": "R007_inlet_turbulence_followup",
            "stage": "native_case_generation_then_run",
            "enabled_now": not gpu_blocked,
            "evidence_type": "blocked_until_gpu_ready" if gpu_blocked else "newly_run_when_executed",
            "purpose": "Retest the default-off AF_caseE-k full-plane inlet follow-up at the best diagnostic no-SGS scale.",
            "trigger_condition": "GPU ready and official preflight passes; inlet follow-up codegen gate must remain pass/fail closed.",
            "command": "python docs/experiments/casee/tools/generate_native_casee.py --dx 2 --steps 48000 --spinup 12000 --sample-dt 2000 --ground-offset-cells 1 --origin-z-offset-m 1.0 --nu-lbm 0.001 --domain-x 4 --domain-y 1 --domain-z 1 --inlet-turbulence-mode k_synthetic_fullplane --inlet-turbulence-scale 2.00 --no-subgrid",
            "expected_artifact": "docs/experiments/casee/native_cases/<run_id>/casee_probe_time_mean.csv",
            "formal_result_policy": "May inform defaults only if official raw_trilinear metrics improve and Case A smoke regression passes.",
            "pass_condition": "Official raw_trilinear metric improves without relying on diagnostic sampling or z-offset substitution; R2 must become positive before any formal accuracy claim.",
            "forbidden_claim": common_forbidden,
        },
        {
