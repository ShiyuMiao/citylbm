...
OFFICIAL_RESIDUAL_DIAGNOSTICS_COMPONENT = ROOT / "CityLBM" / "src" / "Components" / "Results" / "CaseEOfficialResidualDiagnosticsComponent.cs"
...
def build_rows() -> List[Dict[str, Any]]:
    ...
    official_residual_diagnostics_component = read_text(OFFICIAL_RESIDUAL_DIAGNOSTICS_COMPONENT)
    ...
    official_residual_diagnostics_component_gate = read_json(RESULTS_DIR / "citylbm_casee_official_residual_diagnostics_component_gate.json")
    official_residual_diagnostics_binary_gate = read_json(RESULTS_DIR / "citylbm_casee_official_residual_diagnostics_binary_gate.json")
    ...
    rows.append(
        row(
            feedback_id="SF057",
            experiment="Experiment 2 / native Case E inlet-turbulence follow-up codegen",
            finding=(
                "The native Case E generator already provides a default-off AF_caseE-k full-plane inlet "
                "follow-up path, and the next-run plan now replaces the R007 TODO with a concrete no-SGS "
                "scale-2.00 official-height candidate."
            ),
            evidence_type=str(inlet_followup_codegen.get("evidence_type", "missing")),
            source_paths=[
                CASEE_DIR / "tools" / "generate_native_casee.py",
                CASEE_DIR / "tools" / "casee_inlet_followup_codegen_gate.py",
                CASEE_DIR / "tools" / "casee_candidate_sweep_plan.py",
                CASEE_DIR / "tools" / "casee_next_experiment_runbook.py",
                RESULTS_DIR / "casee_inlet_followup_codegen_gate.json",
                RESULTS_DIR / "casee_inlet_followup_codegen_gate.csv",
                RESULTS_DIR / "casee_inlet_followup_codegen_gate.md",
                RESULTS_DIR / "casee_candidate_sweep_plan.json",
                RESULTS_DIR / "casee_next_experiment_runbook.json",
            ],
            decision_class="native_inlet_followup_codegen_no_accuracy_promotion",
            citylbm_status="implemented_default_off_native_inlet_followup_codegen"
            if inlet_followup_codegen.get("inlet_followup_codegen_gate_passed") is True
            and inlet_followup_codegen.get("formal_accuracy_claim_supported") is False
            else "native_inlet_followup_codegen_missing_or_failed",
            implementation_evidence=(
                f"gate_passed={inlet_followup_codegen.get('inlet_followup_codegen_gate_passed')}; "
                f"candidate_count={candidate_sweep.get('candidate_count')}; "
                f"executable_now_count={candidate_sweep.get('executable_now_count')}; "
                f"recommended_tag={candidate_sweep.get('recommended_tag')}"
            ),
            default_setting_allowed=False,
            paper_use="Use as pre-registered software implementation evidence for the next AF-k inlet/no-SGS official follow-up.",
            limitations="Code-generation and planning evidence only; no new inlet follow-up run has completed, no R2 changed, and no inlet/SGS setting can be promoted before an official z2m release-gate pass.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF060",
            experiment="Experiment 2 / next-run runbook native codegen preflight",
            finding=(
                "The next-run official follow-up commands R005, R006, R007, R008, and R010 are now "
                "machine-preflighted by executing only their native case-generation step, verifying official "
                "manifest fields, diagnostic-default blockers, path length, and cleanup before any long solver run."
            ),
            evidence_type=str(runbook_codegen_preflight.get("evidence_type", "missing")),
            source_paths=[
                CASEE_DIR / "tools" / "casee_runbook_codegen_preflight.py",
                CASEE_DIR / "tools" / "casee_next_experiment_runbook.py",
                CASEE_DIR / "tools" / "generate_native_casee.py",
                RESULTS_DIR / "casee_runbook_codegen_preflight.json",
                RESULTS_DIR / "casee_runbook_codegen_preflight.csv",
                RESULTS_DIR / "casee_runbook_codegen_preflight.md",
                RESULTS_DIR / "casee_next_experiment_runbook.json",
            ],
            decision_class="runbook_codegen_preflight_no_accuracy_promotion",
            citylbm_status="implemented_runbook_codegen_preflight"
            if runbook_codegen_preflight.get("runbook_codegen_preflight_passed") is True
            and runbook_codegen_preflight.get("formal_accuracy_claim_supported") is False
            else "runbook_codegen_preflight_missing_or_failed",
            implementation_evidence=(
                f"gate_passed={runbook_codegen_preflight.get('runbook_codegen_preflight_passed')}; "
                f"required_ids_present={runbook_codegen_preflight.get('required_runbook_ids_present')}; "
                f"covered_ids={','.join(runbook_codegen_preflight.get('covered_runbook_ids') or [])}"
            ),
            default_setting_allowed=False,
            paper_use="Use as runbook reproducibility evidence that official follow-up inputs are executable and claim-gated before solver scheduling.",
            limitations="Input-generation evidence only; no FluidX3D run, probe CSV, official metric update, R2 improvement, or default accuracy promotion is supported.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF061",
            experiment="Experiment 2 / Case E default-promotion gate",
            finding=(
                "A dedicated default-promotion gate now converts the official metric, Case A, Rhino/GHA load, "
                "raw-trilinear protocol, no-fitting, and traceability requirements into per-setting blockers so "
                "diagnostic controls cannot silently become CityLBM defaults."
            ),
            evidence_type=str(default_promotion.get("evidence_type", "missing")),
            source_paths=[
                CASEE_DIR / "tools" / "casee_default_promotion_gate.py",
                RESULTS_DIR / "casee_default_promotion_gate.json",
                RESULTS_DIR / "casee_default_promotion_gate.csv",
                RESULTS_DIR / "casee_default_promotion_gate.md",
                RESULTS_DIR / "release_gate.json",
                RESULTS_DIR / "casee_default_policy_gate.json",
            ],
            decision_class="default_promotion_gate_no_accuracy_promotion",
            citylbm_status="implemented_default_promotion_gate"
            if default_promotion.get("default_promotion_gate_passed") is True
            and default_promotion.get("any_diagnostic_default_promotion_allowed") is False
            else "default_promotion_gate_missing_or_failed",
            implementation_evidence=(
                f"gate_passed={default_promotion.get('default_promotion_gate_passed')}; "
                f"any_promotion_allowed={default_promotion.get('any_diagnostic_default_promotion_allowed')}; "
                f"row_count={len(default_promotion.get('rows') or [])}"
            ),
            default_setting_allowed=False,
            paper_use="Use as software-implications evidence for why diagnostic improvements remain experimental until all formal release gates pass.",
            limitations="Promotion-policy evidence only; no FluidX3D run, official metric update, R2 improvement, or default accuracy promotion is supported.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF062",
            experiment="Experiment 2 / Case E Rhino load evidence packet gate",
            finding=(
                "The Rhino/GHA manual load handoff now has a packet gate that writes expected version, path, "
                "SHA256, screenshot/log requirements, post-capture commands, and forbidden interpretations while "
                "remaining fail-closed until a real Rhino/Grasshopper manifest and artifacts exist."
            ),
            evidence_type=str(rhino_evidence_packet_gate.get("evidence_type", "missing")),
            source_paths=[
                CASEE_DIR / "tools" / "casee_rhino_load_evidence_packet_gate.py",
                RESULTS_DIR / "casee_rhino_load_evidence_packet_gate.json",
                RESULTS_DIR / "casee_rhino_load_evidence_packet_gate.csv",
                RESULTS_DIR / "casee_rhino_load_evidence_packet_gate.md",
                RESULTS_DIR / "rhino_gha_load_manifest.expected.json",
                RESULTS_DIR / "rhino_gha_load_gate.json",
            ],
            decision_class="manual_rhino_load_evidence_packet_no_accuracy_promotion",
            citylbm_status="implemented_rhino_load_evidence_packet_gate"
            if rhino_evidence_packet_gate.get("rhino_load_evidence_packet_gate_passed") is True
            else "rhino_load_evidence_packet_gate_missing_or_failed",
            implementation_evidence=(
                f"packet_gate_passed={rhino_evidence_packet_gate.get('rhino_load_evidence_packet_gate_passed')}; "
                f"manual_claim_ready={rhino_evidence_packet_gate.get('manual_rhino_load_claim_ready')}; "
                f"rhino_loaded_new_gha={rhino_evidence_packet_gate.get('rhino_loaded_new_gha')}; "
                f"expected_sha={rhino_evidence_packet_gate.get('expected_gha_sha256')}"
            ),
            default_setting_allowed=False,
            paper_use="Use as a software-load protocol-risk mitigation showing exactly how old-GHA contamination is prevented before formal claims.",
            limitations="Manual evidence packet only; it does not prove Rhino loaded the plugin, run FluidX3D, improve official metrics, or permit formal v0.4.0.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF064",
            experiment="Experiment 2 / Case E accuracy action plan gate",
            finding=(
                "The quantified accuracy gap is now converted into an ordered action plan that separates "
                "release safety, Rhino/GHA evidence, environment recovery, wall-model follow-up, AF-k/no-SGS "
                "follow-up, C016 channel-response follow-up, post-run audit, and post-hoc calibration rejection."
            ),
            evidence_type=str(accuracy_action_plan.get("evidence_type", "missing")),
            source_paths=[
                CASEE_DIR / "tools" / "casee_accuracy_action_plan_gate.py",
                RESULTS_DIR / "casee_accuracy_action_plan_gate.json",
                RESULTS_DIR / "casee_accuracy_action_plan_gate.csv",
                RESULTS_DIR / "casee_accuracy_action_plan_gate.md",
                RESULTS_DIR / "casee_research_accuracy_gap_gate.json",
                RESULTS_DIR / "casee_next_experiment_runbook.json",
            ],
            decision_class="accuracy_action_plan_no_default_promotion",
            citylbm_status="implemented_accuracy_action_plan_gate"
            if accuracy_action_plan.get("accuracy_action_plan_gate_passed") is True
            else "accuracy_action_plan_gate_missing_or_failed",
            implementation_evidence=(
                f"action_plan_passed={accuracy_action_plan.get('accuracy_action_plan_gate_passed')}; "
                f"enabled_now_count={accuracy_action_plan.get('enabled_now_count')}; "
                f"official_followup_action_count={accuracy_action_plan.get('official_followup_action_count')}; "
                f"recommended_tag={accuracy_action_plan.get('recommended_tag')}"
            ),
            default_setting_allowed=False,
            paper_use="Use as a reviewer-facing bridge from current accuracy limitations to the next official CFD actions and software policy.",
            limitations="Action-plan evidence only; it does not run FluidX3D, improve metrics, or permit formal v0.4.0.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF067",
            experiment="Experiment 2 / CityLBM plugin Case E paper claim-card component",
            finding=(
                "CityLBM now exposes a Grasshopper Case E Paper Claim Card component that separates "
                "paper-ready negative-validation statements, limitations, forbidden claims, and evidence paths."
            ),
            evidence_type=str(paper_claim_card_component_gate.get("evidence_type", "missing")),
            source_paths=[
                PAPER_CLAIM_CARD_COMPONENT,
                CASEE_DIR / "tools" / "citylbm_casee_paper_claim_card_component_gate.py",
                RESULTS_DIR / "citylbm_casee_paper_claim_card_component_gate.json",
                RESULTS_DIR / "citylbm_casee_paper_claim_card_component_gate.csv",
                RESULTS_DIR / "citylbm_casee_paper_claim_card_component_gate.md",
                RESULTS_DIR / "casee_paper_evidence_gate.json",
                RESULTS_DIR / "casee_publication_readiness_gate.json",
            ],
            decision_class="software_paper_claim_card_component_no_accuracy_promotion",
            citylbm_status="implemented_casee_paper_claim_card_component"
            if paper_claim_card_component_gate.get("casee_paper_claim_card_component_gate_passed") is True
            and paper_claim_card_component_gate.get("formal_accuracy_claim_supported") is False
            and paper_claim_card_component_gate.get("default_setting_allowed") is False
            else "casee_paper_claim_card_component_missing_or_failed",
            implementation_evidence=(
                f"source_gate_passed={paper_claim_card_component_gate.get('casee_paper_claim_card_component_gate_passed')}; "
                f"component_has_negative_validation={'negative-validation result' in paper_claim_card_component}; "
                f"component_has_limitations={'official z=2 m R2 remains negative' in paper_claim_card_component}; "
                f"component_blocks_predictive_accuracy={'Do not claim predictive accuracy' in paper_claim_card_component}; "
                f"component_has_evidence_paths={'casee_paper_evidence_gate.json' in paper_claim_card_component}"
            ),
            default_setting_allowed=False,
            paper_use="Use as plugin-side paper-writing support that keeps Case E claims within the verified negative-validation boundary.",
            limitations="Plugin component evidence only; it does not run FluidX3D, compute a new R2, improve official z2m metrics, promote defaults, or permit formal v0.4.0.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF068",
            experiment="Experiment 2 / packaged CityLBM Case E paper claim-card component",
            finding=(
                "The packaged CityLBM GHA is audited for the Case E Paper Claim Card component markers, "
                "including paper-ready claims, limitations, forbidden claims, and evidence-path outputs."
            ),
            evidence_type=str(paper_claim_card_binary_gate.get("evidence_type", "missing")),
            source_paths=[
                CASEE_DIR / "tools" / "citylbm_casee_paper_claim_card_binary_gate.py",
                RESULTS_DIR / "citylbm_casee_paper_claim_card_binary_gate.json",
                RESULTS_DIR / "citylbm_casee_paper_claim_card_binary_gate.csv",
                RESULTS_DIR / "citylbm_casee_paper_claim_card_binary_gate.md",
                ROOT / "CityLBM" / "bin" / "CityLBM.gha",
                ROOT / "CityLBM" / "bin" / "Release" / "CityLBM.gha",
            ],
            decision_class="packaged_paper_claim_card_component_no_accuracy_promotion",
            citylbm_status="implemented_packaged_casee_paper_claim_card_component"
            if paper_claim_card_binary_gate.get("casee_paper_claim_card_binary_gate_passed") is True
            and paper_claim_card_binary_gate.get("formal_accuracy_claim_supported") is False
            and paper_claim_card_binary_gate.get("default_setting_allowed") is False
            else "casee_paper_claim_card_binary_gate_missing_or_failed",
            implementation_evidence=(
                f"binary_gate_passed={paper_claim_card_binary_gate.get('casee_paper_claim_card_binary_gate_passed')}; "
                f"tracked_gha_sha256={paper_claim_card_binary_gate.get('tracked_gha_sha256')}; "
                f"tracked_matches_release={((paper_claim_card_binary_gate.get('checks') or {}).get('tracked_gha_matches_release_gha'))}; "
                f"markers_present={((paper_claim_card_binary_gate.get('checks') or {}).get('all_required_markers_present_in_tracked_gha'))}"
            ),
            default_setting_allowed=False,
            paper_use="Use as packaged-plugin evidence that the release asset contains the paper claim-card component.",
            limitations="Packaged GHA evidence only; it does not prove Rhino loaded the plugin, run FluidX3D, improve official z2m metrics, promote defaults, or permit formal v0.4.0.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF071",
            experiment="Experiment 2 / CityLBM plugin Case E official metric-gate component",
            finding=(
                "CityLBM now exposes a Grasshopper Case E Official Metric Gate component that reports "
                "the formal z=2 m official MAE/RMSE/bias/R2/Pearson values, release thresholds, metric "
                "checks, and forbidden claims directly in the plugin workflow."
            ),
            evidence_type=str(official_metric_gate_component_gate.get("evidence_type", "missing")),
            source_paths=[
                OFFICIAL_METRIC_GATE_COMPONENT,
                CASEE_DIR / "tools" / "citylbm_casee_official_metric_gate_component_gate.py",
                RESULTS_DIR / "citylbm_casee_official_metric_gate_component_gate.json",
                RESULTS_DIR / "citylbm_casee_official_metric_gate_component_gate.csv",
                RESULTS_DIR / "citylbm_casee_official_metric_gate_component_gate.md",
                RESULTS_DIR / "release_gate.json",
                RESULTS_DIR / "casee_metrics.csv",
                RESULTS_DIR / "casee_validation_report.md",
            ],
            decision_class="software_official_metric_gate_component_no_accuracy_promotion",
            citylbm_status="implemented_casee_official_metric_gate_component"
            if official_metric_gate_component_gate.get("casee_official_metric_gate_component_gate_passed") is True
            and official_metric_gate_component_gate.get("formal_accuracy_claim_supported") is False
            and official_metric_gate_component_gate.get("default_setting_allowed") is False
            else "casee_official_metric_gate_component_missing_or_failed",
            implementation_evidence=(
                f"source_gate_passed={official_metric_gate_component_gate.get('casee_official_metric_gate_component_gate_passed')}; "
                f"component_has_official_metrics={all(marker in official_metric_gate_component for marker in ['21.111408125', '27.72103208243715', '-16.409216', '-2.006330362229977', '0.11575649438573923'])}; "
                f"component_has_thresholds={all(marker in official_metric_gate_component for marker in ['MAE threshold: < 15.0 pp', 'R2 threshold: > 0.0', 'Pearson threshold: > 0.0'])}; "
                f"component_has_gate_checks={'official_z2m_metric_gate=' in official_metric_gate_component and 'formal_release_allowed=false' in official_metric_gate_component}; "
                f"component_blocks_claims={'Do not claim predictive accuracy' in official_metric_gate_component}"
            ),
            default_setting_allowed=False,
            paper_use="Use as software workflow evidence that the plugin exposes the formal official metric failure and threshold boundary to Grasshopper users.",
            limitations="Plugin component evidence only; it does not run FluidX3D, compute a new R2, improve official z2m metrics, promote defaults, prove Rhino load, or permit formal v0.4.0.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF072",
            experiment="Experiment 2 / packaged CityLBM Case E official metric-gate component",
            finding=(
                "The packaged CityLBM GHA is audited for the Case E Official Metric Gate component markers, "
                "including official metric values, thresholds, metric checks, forbidden claims, and component GUID."
            ),
            evidence_type=str(official_metric_gate_binary_gate.get("evidence_type", "missing")),
            source_paths=[
                CASEE_DIR / "tools" / "citylbm_casee_official_metric_gate_binary_gate.py",
                RESULTS_DIR / "citylbm_casee_official_metric_gate_binary_gate.json",
                RESULTS_DIR / "citylbm_casee_official_metric_gate_binary_gate.csv",
                RESULTS_DIR / "citylbm_casee_official_metric_gate_binary_gate.md",
                ROOT / "CityLBM" / "bin" / "CityLBM.gha",
                ROOT / "CityLBM" / "bin" / "Release" / "CityLBM.gha",
            ],
            decision_class="packaged_official_metric_gate_component_no_accuracy_promotion",
            citylbm_status="implemented_packaged_casee_official_metric_gate_component"
            if official_metric_gate_binary_gate.get("casee_official_metric_gate_binary_gate_passed") is True
            and official_metric_gate_binary_gate.get("formal_accuracy_claim_supported") is False
            and official_metric_gate_binary_gate.get("default_setting_allowed") is False
            else "casee_official_metric_gate_binary_gate_missing_or_failed",
            implementation_evidence=(
                f"binary_gate_passed={official_metric_gate_binary_gate.get('casee_official_metric_gate_binary_gate_passed')}; "
                f"tracked_gha_sha256={official_metric_gate_binary_gate.get('tracked_gha_sha256')}; "
                f"tracked_matches_release={((official_metric_gate_binary_gate.get('checks') or {}).get('tracked_gha_matches_release_gha'))}; "
                f"markers_present={((official_metric_gate_binary_gate.get('checks') or {}).get('all_required_markers_present_in_tracked_gha'))}"
            ),
            default_setting_allowed=False,
            paper_use="Use as packaged-plugin evidence that the release asset contains the official metric-gate component.",
            limitations="Packaged GHA evidence only; it does not prove Rhino loaded the plugin, run FluidX3D, improve official z2m metrics, promote defaults, or permit formal v0.4.0.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF075",
            experiment="Experiment 2 / CityLBM plugin Case E official residual diagnostics",
            finding=(
                "CityLBM now exposes a Grasshopper Case E Official Residual Diagnostics component that reports "
                "top absolute-residual probes, observed-speed group summaries, under-prediction fractions, and "
                "protocol risks from a supplied official probe CSV."
            ),
            evidence_type=str(official_residual_diagnostics_component_gate.get("evidence_type", "missing")),
            source_paths=[
                OFFICIAL_RESIDUAL_DIAGNOSTICS_COMPONENT,
                CASEE_DIR / "tools" / "citylbm_casee_official_residual_diagnostics_component_gate.py",
                RESULTS_DIR / "citylbm_casee_official_residual_diagnostics_component_gate.json",
                RESULTS_DIR / "citylbm_casee_official_residual_diagnostics_component_gate.csv",
                RESULTS_DIR / "citylbm_casee_official_residual_diagnostics_component_gate.md",
                RESULTS_DIR / "casee_c014_residual_structure_audit.json",
                RESULTS_DIR / "casee_validation_report.md",
            ],
            decision_class="software_official_residual_diagnostics_component_no_accuracy_promotion",
            citylbm_status="implemented_casee_official_residual_diagnostics_component"
            if official_residual_diagnostics_component_gate.get("casee_official_residual_diagnostics_component_gate_passed") is True
            and official_residual_diagnostics_component_gate.get("formal_accuracy_claim_supported") is False
            and official_residual_diagnostics_component_gate.get("default_setting_allowed") is False
            else "casee_official_residual_diagnostics_component_missing_or_failed",
            implementation_evidence=(
                f"source_gate_passed={official_residual_diagnostics_component_gate.get('casee_official_residual_diagnostics_component_gate_passed')}; "
                f"component_has_top_rows={'Top Residual Rows' in official_residual_diagnostics_component}; "
                f"component_has_group_rows={all(marker in official_residual_diagnostics_component for marker in ['low_official_speed', 'mid_official_speed', 'high_official_speed', 'under_fraction'])}; "
                f"component_blocks_claims={'residual_diagnostics_do_not_improve_metrics=true' in official_residual_diagnostics_component and 'posthoc_calibration_not_validation=true' in official_residual_diagnostics_component}"
            ),
            default_setting_allowed=False,
            paper_use="Use as plugin-side residual-structure evidence for explaining why official Case E z=2 m validation remains negative.",
            limitations="Residual diagnostics only; it does not run FluidX3D, improve official z2m metrics, prove Rhino load, promote defaults, or permit formal v0.4.0.",
        )
    )
    ...
    rows.append(
        row(
            feedback_id="SF076",
            experiment="Experiment 2 / packaged CityLBM Case E official residual diagnostics",
            finding=(
                "The packaged CityLBM GHA is audited for the Case E Official Residual Diagnostics component markers, "
                "including top residual rows, observed-speed groups, risk rows, and forbidden-claim boundary strings."
            ),
            evidence_type=str(official_residual_diagnostics_binary_gate.get("evidence_type", "missing")),
            source_paths=[
                CASEE_DIR / "tools" / "citylbm_casee_official_residual_diagnostics_binary_gate.py",
                RESULTS_DIR / "citylbm_casee_official_residual_diagnostics_binary_gate.json",
                RESULTS_DIR / "citylbm_casee_official_residual_diagnostics_binary_gate.csv",
                RESULTS_DIR / "citylbm_casee_official_residual_diagnostics_binary_gate.md",
                ROOT / "CityLBM" / "bin" / "CityLBM.gha",
                ROOT / "CityLBM" / "bin" / "Release" / "CityLBM.gha",
            ],
            decision_class="packaged_official_residual_diagnostics_component_no_accuracy_promotion",
            citylbm_status="implemented_packaged_casee_official_residual_diagnostics_component"
            if official_residual_diagnostics_binary_gate.get("casee_official_residual_diagnostics_binary_gate_passed") is True
            and official_residual_diagnostics_binary_gate.get("formal_accuracy_claim_supported") is False
            and official_residual_diagnostics_binary_gate.get("default_setting_allowed") is False
            else "casee_official_residual_diagnostics_binary_gate_missing_or_failed",
            implementation_evidence=(
                f"binary_gate_passed={official_residual_diagnostics_binary_gate.get('casee_official_residual_diagnostics_binary_gate_passed')}; "
                f"tracked_gha_sha256={official_residual_diagnostics_binary_gate.get('tracked_gha_sha256')}; "
                f"tracked_matches_release={((official_residual_diagnostics_binary_gate.get('checks') or {}).get('tracked_gha_matches_release_gha'))}; "
                f"markers_present={((official_residual_diagnostics_binary_gate.get('checks') or {}).get('all_required_markers_present_in_tracked_gha'))}"
            ),
            default_setting_allowed=False,
            paper_use="Use as packaged-plugin evidence that the release asset contains the residual diagnostics component.",
            limitations="Packaged GHA evidence only; it does not prove Rhino loaded the plugin, run FluidX3D, improve official z2m metrics, promote defaults, or permit formal v0.4.0.",
        )
    )
...
def main() -> int:
    ...
    payload = {
        ...
        "source_artifacts": [
            ...
            rel(RESULTS_DIR / "casee_default_promotion_gate.json"),
            ...
            rel(OFFICIAL_RESIDUAL_DIAGNOSTICS_COMPONENT),
        ],
    }
