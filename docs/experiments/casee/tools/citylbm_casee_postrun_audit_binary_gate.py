#!/usr/bin/env python3
"""Audit that the packaged CityLBM GHA contains the Case E Post-run Audit component."""
...
SOURCE_GATE = RESULTS_DIR / "citylbm_casee_postrun_audit_component_gate.json"
OUT_JSON = RESULTS_DIR / "citylbm_casee_postrun_audit_binary_gate.json"
OUT_CSV = RESULTS_DIR / "citylbm_casee_postrun_audit_binary_gate.csv"
OUT_MD = RESULTS_DIR / "citylbm_casee_postrun_audit_binary_gate.md"


REQUIRED_MARKERS = [
    "CaseEPostRunAuditComponent",
    "Case E Post-run Audit",
    "Audit Command",
    "Ready For Official Audit",
    "Formal Result Allowed Now",
    "Candidate SHA256",
    "raw_trilinear",
    "casee_audit.py",
    "does not permit formal v0.4.0",
    "19B94D68-EB71-41C0-B4AB-35DAFECE4079",
]
...
def build_payload() -> Dict[str, Any]:
    ...
    checks = {
        ...
        "source_component_gate_passed": source_gate.get("casee_postrun_audit_component_gate_passed") is True,
    ...
    return {
        ...
        "claim_readiness": "paper_ready_packaged_casee_postrun_audit_component" if passed else "blocked_packaged_casee_postrun_audit_component",
        "casee_postrun_audit_binary_gate_passed": passed,
        ...
        "checks": checks,
        "boundary": (
            "This gate checks that the packaged GHA contains the Case E post-run audit component strings. "
            "It is software packaging evidence only; it does not prove Rhino loaded the plugin, run CFD, "
            "update official metrics, or permit formal v0.4.0."
        ),
    }
...
def write_markdown(path: Path, payload: Dict[str, Any]) -> None:
    lines = [
        ...
        "## Verdict",
        "",
        f"- Gate passed: {payload['casee_postrun_audit_binary_gate_passed']}",
...
def main() -> int:
    ...
    print(json.dumps({"casee_postrun_audit_binary_gate_passed": payload["casee_postrun_audit_binary_gate_passed"], "out_json": rel(OUT_JSON)}, indent=2))
    return 0 if payload["casee_postrun_audit_binary_gate_passed"] else 1
