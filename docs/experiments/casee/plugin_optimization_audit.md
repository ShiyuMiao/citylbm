...
- The native Case E generator now has a compile-verified probe-mode diagnostic runner that writes formal `raw_trilinear` plus `nearest_valid`, `fluid_weighted`, `vertical_valid_above`, and `z_plus_half` diagnostic columns. This is compile-only evidence until a full FluidX3D run completes; no new accuracy metric is claimed from it.
