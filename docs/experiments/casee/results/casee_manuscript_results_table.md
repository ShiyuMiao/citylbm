...
Generated: 2026-08-13T11:21:42.838352+00:00
...
| `software_traceability_status` | paper_ready_manifest_traceability | 24 |  |  |  | CityLBM exposes and audits the run manifest path so protocol and claim-boundary metadata are traceable from the Grasshopper workflow. |
| `release_boundary_status` | blocked_formal_release_gate |  |  |  |  | The formal release gate remains closed (`formal_release_allowed=False`), and the recommended tag is `v0.4.0-rc87`. |
