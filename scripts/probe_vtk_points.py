...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--domain-origin",
        default="",
        help="Optional domain_origin.json. When provided, official meter coordinates are mapped to VTK lattice coordinates.",
    )
...
def read_domain_origin(path_text: str) -> Dict[str, float]:
    if not path_text.strip():
        return {}
    path = Path(path_text).resolve()
    if not path.exists():
        raise SystemExit(f"domain_origin.json not found: {path}")
    with path.open("r", encoding="utf-8-sig") as handle:
        raw = json.load(handle)
    dx = as_float(raw.get("Dx"))
    if dx is None or dx <= 0.0:
        raise SystemExit(f"domain_origin.json has invalid Dx: {path}")
    result = {"Dx": float(dx)}
    for axis in ("X", "Y", "Z"):
        value = as_float(raw.get(f"DomainMin{axis}", raw.get(f"DomainOrigin{axis}")))
        if value is None:
            raise SystemExit(f"domain_origin.json missing DomainMin{axis}/DomainOrigin{axis}: {path}")
        result[f"DomainMin{axis}"] = float(value)
    return result


def map_world_to_vtk_coordinate(
    official_point: Tuple[float, float, float],
    domain_origin: Dict[str, float],
    vtk_origin: Tuple[float, float, float],
) -> Tuple[Tuple[float, float, float], str]:
    if not domain_origin:
        return official_point, "identity"
    dx = domain_origin["Dx"]
    domain_min = (
        domain_origin["DomainMinX"],
        domain_origin["DomainMinY"],
        domain_origin["DomainMinZ"],
    )
    mapped = tuple((official_point[axis] - domain_min[axis]) / dx + vtk_origin[axis] for axis in range(3))
    return mapped, "domain_origin_json_world_m_to_vtk_lattice"
...
def main() -> int:
    ...
    domain_origin = read_domain_origin(args.domain_origin)
    ...
    for index, row in enumerate(official_rows):
            ...
            out_rows.append(
                {
                    ...
                    "coordinate_mapping": "invalid_coordinate",
                    **official_summary,
                    "u": "",
                    "v": "",
                    "w": "",
        ...
        point, coordinate_mapping = map_world_to_vtk_coordinate(official_point, domain_origin, origin)
