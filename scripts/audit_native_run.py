...
def required_average_last_n_for_step_span(
    ...
    if save_interval is None or save_interval <= 0 or min_avg_step_span <= 0:
        return max(1, min_avg_frames)
    return max(min_avg_frames, int(math.ceil(min_avg_step_span / float(save_interval))) + 1)
...
def expected_vtk_frame_preflight(
    ...
    recommended_average_last_n = required_average_last_n_for_step_span(
        requested_save_interval,
        min_avg_frames,
        min_avg_step_span,
    )
    ...
    if requested_time_steps is not None and requested_save_interval is not None and requested_save_interval > 0:
        ...
        if start < 0:
            ...
            if requested_save_interval > 0:
                recommended_minimum_time_steps = start + (recommended_average_last_n - 1) * requested_save_interval
    ...
    return {
        ...
        "recommended_average_last_n_for_step_span": recommended_average_last_n,
        "recommended_minimum_time_steps_for_current_save_interval": recommended_minimum_time_steps,
        "adaptive_average_window_policy": "increase average_last_n when save_interval is too small for the minimum final-window step span",
...
def build_audit(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    audit: Dict[str, Any] = {
        ...
        "recommended_average_last_n_for_step_span": requested_frame_preflight["recommended_average_last_n_for_step_span"],
        "recommended_minimum_time_steps_for_current_save_interval": requested_frame_preflight["recommended_minimum_time_steps_for_current_save_interval"],
        "adaptive_average_window_policy": requested_frame_preflight["adaptive_average_window_policy"],
