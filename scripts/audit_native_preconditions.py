...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--official-wind-filter", default="", help="Optional official RS wind-direction filter. Defaults to --wind-direction-label.")
...
def build_native_precondition_closure(reasons: List[str]) -> Dict[str, Any]:
    ...
    stage_specs = [
        ...
        (
            ...
            "Complete validation protocol audit after the primary physics and runtime blockers are explicit.",
        ),
    ]
...
def manifest_case_to_run_source_parity(manifest: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for role in required_roles:
        ...
        if match is not True:
            if as_bool(item.get("AllowedMismatch") if "AllowedMismatch" in item else item.get("allowed_mismatch")):
                allowed_mismatch_pair_count += 1
                if len(case_sha) == 64 and len(source_sha) == 64:
    ...
    return {
        ...
        "allowed_mismatch_pair_count": allowed_mismatch_pair_count,
...
def build_official_coordinate_lookup(
    ...
    effective_case = condition_filter.strip() or case
    effective_wind = wind_filter.strip() or wind_direction
    rows, filter_error = filter_official_rows(rows, effective_case, effective_wind)
    if filter_error:
        return {}, filter_error
...
def main() -> int:
    ...
    runtime_inlet_diagnostics = manifest.get("RuntimeInletDiagnosticsGate", {})
    if not isinstance(runtime_inlet_diagnostics, dict):
        runtime_inlet_diagnostics = {}
    runtime_inlet_diagnostics_gate = str(runtime_inlet_diagnostics.get("Gate") or "").strip().lower()
    runtime_inlet_diagnostics_reasons = split_scalar_list(runtime_inlet_diagnostics.get("Reasons"))
    if not runtime_inlet_diagnostics_reasons:
    ...
    if runtime_inlet_diagnostics and runtime_inlet_diagnostics_gate not in {"pass", "not_applicable"}:
        ...
        for reason in runtime_inlet_diagnostics_reasons:
            reasons.append(f"runtime_inlet_diagnostics_reason:{reason}")
    if runtime_inlet_diagnostics_requested is True and runtime_inlet_diagnostics_gate != "pass":
        reasons.append("runtime_inlet_diagnostics_required_but_not_pass")
    ...
    flow_through_time = manifest.get("FlowThroughTimeGate", {})
    if not isinstance(flow_through_time, dict):
        flow_through_time = {}
    flow_through_gate = str(flow_through_time.get("Gate") or "").strip().lower()
    flow_through_reasons = split_scalar_list(flow_through_time.get("Reasons"))
    if not flow_through_reasons:
        flow_through_reasons = split_scalar_list(flow_through_time.get("ReasonsCsv"))
    flow_through_minimum_count = as_float(flow_through_time.get("MinimumFlowThroughCount"))
    flow_through_planned_count = as_float(flow_through_time.get("PlannedFlowThroughCount"))
    flow_through_one_steps = as_int(flow_through_time.get("EstimatedOneFlowThroughSteps"))
    flow_through_minimum_steps = as_int(flow_through_time.get("RecommendedMinimumTimeStepsForFlowThrough"))
    flow_through_domain_cells = as_int(flow_through_time.get("DomainLengthCells"))
    flow_through_reference_velocity = as_float(flow_through_time.get("ReferenceVelocityLbm"))
    flow_through_axis = str(flow_through_time.get("DominantAxis") or "").strip()
    flow_through_time_evidence_reasons: List[str] = []
    if not flow_through_time:
        flow_through_time_evidence_reasons.append("flow_through_time_gate_missing")
    elif flow_through_gate not in {"pass", "not_applicable"}:
        flow_through_time_evidence_reasons.append(
            f"flow_through_time_gate_not_pass:{flow_through_gate or 'missing'}"
        )
        for reason in flow_through_reasons:
            flow_through_time_evidence_reasons.append(f"flow_through_time_reason:{reason}")
    if flow_through_time_evidence_reasons:
        reasons.extend(flow_through_time_evidence_reasons)
    ...
    result = {
        ...
        "flow_through_time_gate": flow_through_gate,
        "flow_through_time_gate_reasons": flow_through_reasons,
        "flow_through_time_gate_reasons_csv": ";".join(flow_through_reasons),
        "flow_through_minimum_count": flow_through_minimum_count,
        "flow_through_planned_count": flow_through_planned_count,
        "flow_through_estimated_one_flowthrough_steps": flow_through_one_steps,
        "flow_through_recommended_minimum_time_steps": flow_through_minimum_steps,
        "flow_through_domain_length_cells": flow_through_domain_cells,
        "flow_through_reference_velocity_lbm": flow_through_reference_velocity,
        "flow_through_dominant_axis": flow_through_axis,
        ...
        "runtime_inlet_diagnostics_gate": runtime_inlet_diagnostics_gate,
        "runtime_inlet_diagnostics_gate_reasons": runtime_inlet_diagnostics_reasons,
        ...
        **boundary_source_hash_check,
        ...
        "case_to_run_source_parity_allowed_mismatch_pair_count": source_parity_audit["allowed_mismatch_pair_count"],
