...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--average-last-n", type=int, default=40, help="Use last N frames.")
    ...
    parser.add_argument(
        "--velocity-scale",
        default="auto",
        help="Multiply VTK velocities by this scale. Use 'auto' to read VelocityScaleLbmToMps from metadata.",
    )
...
def resolve_velocity_scale(args: argparse.Namespace, metadata: Dict[str, Any]) -> Tuple[float, str]:
    ...
    if scale is None or scale <= 0.0:
        raise SystemExit(f"Invalid --velocity-scale: {args.velocity_scale}")
    return scale, "cli"


def custom_profile_z_bounds(metadata: Dict[str, Any]) -> Tuple[Optional[float], Optional[float], str]:
    profiles = metadata.get("CustomProfile")
    if not isinstance(profiles, list):
        return None, None, "metadata_custom_profile_absent"
    z_values: List[float] = []
    for item in profiles:
        ...
        if z is not None:
            z_values.append(z)
    if len(z_values) < 2:
        return None, None, "metadata_custom_profile_insufficient_z"
    return min(z_values), max(z_values), "metadata_custom_profile_z_range"
...
def cell_indices(index: int, dims: Tuple[int, int, int]) -> Tuple[int, int, int]:
    nx, ny, _ = dims
    i = index % nx
    j = (index // nx) % ny
    k = index // (nx * ny)
...
def metadata_physical_z(index: int, dims: Tuple[int, int, int], metadata: Dict[str, Any]) -> Optional[float]:
    dx = as_float(metadata.get("DxM"))
    profile_origin = as_float(metadata.get("ProfileOriginZM"))
    if dx is None or dx <= 0.0 or profile_origin is None:
        return None
    _, _, k = cell_indices(index, dims)
    return profile_origin + (float(k) + 0.5) * dx


def target_profile_z(
    index: int,
    ...
    physical_z = metadata_physical_z(index, dims, metadata)
    if physical_z is not None:
        return physical_z, "metadata_profile_origin_dx"
    return coordinate(index, dims, origin, spacing)[2], "vtk_coordinate"
...
def main() -> int:
    ...
    custom_z_min, custom_z_max, custom_z_source = custom_profile_z_bounds(metadata)
    ...
    target_z_sources: List[str] = []
    for idx in selected:
        target_z, target_z_source = target_profile_z(
            idx,
            first["dimensions"],
            first["origin"],
            first["spacing"],
        ...
        target_z_sources.append(target_z_source)
        ...
        if streamwise_mean is None:
            continue
        key = bin_key(target_z, af_samples, args.z_bin_m)
        bucket = bins.setdefault(
            key,
            {"z_values": [], "u_values": [], "k_values": [], "count": 0, "z_source": target_z_source},
        )
        bucket["z_values"].append(target_z)
    ...
    for key in sorted(bins.keys()):
        ...
        if custom_z_min is not None and z_mean < custom_z_min:
            comparison_included = False
            comparison_exclusion_reason = "below_custom_profile_z_range"
        elif custom_z_max is not None and z_mean > custom_z_max:
            comparison_included = False
            comparison_exclusion_reason = "above_custom_profile_z_range"
        if comparison_included and u_error is not None and af_u is not None:
            u_errors.append(u_error)
            af_u_values.append(abs(af_u))
        if comparison_included and k_error is not None and af_k is not None:
            k_errors.append(k_error)
            af_k_values.append(abs(af_k))
        rows.append(
            {
                "z_m": z_mean,
                "z_source": bucket.get("z_source", ""),
                "sample_count": bucket["count"],
                "comparison_included": comparison_included,
    ...
    report = {
        ...
        "comparison_height_bin_count": sum(1 for row in rows if row.get("comparison_included")),
        "excluded_height_bin_count": sum(1 for row in rows if not row.get("comparison_included")),
        "comparison_z_min_m": custom_z_min,
        "comparison_z_max_m": custom_z_max,
        "comparison_z_range_source": custom_z_source,
        "target_profile_z_source": next((source for source in target_z_sources if source), ""),
