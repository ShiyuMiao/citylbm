...
def paper_grade_inlet_method_pass(
    *,
    ...
    audit_has_rms_k_velocity_surrogate: Optional[bool] = None,
    audit_inlet_source_rms_k_surrogate_gate: str = "",
) -> bool:
    return not paper_grade_inlet_method_failure_reasons(
        ...
        audit_has_rms_k_velocity_surrogate=audit_has_rms_k_velocity_surrogate,
        audit_inlet_source_rms_k_surrogate_gate=audit_inlet_source_rms_k_surrogate_gate,
    )
...
def paper_grade_inlet_method_failure_reasons(
    *,
    ...
    if audit_has_rms_k_velocity_surrogate is True:
        reasons.append("audit_has_rms_k_velocity_surrogate_not_false:True")
    elif audit_has_rms_k_velocity_surrogate is None:
        reasons.append("audit_has_rms_k_velocity_surrogate_missing")
    if str(audit_inlet_source_rms_k_surrogate_gate or "").strip().lower() not in {"pass", ""}:
        reasons.append(
            "audit_inlet_source_rms_k_surrogate_gate_not_pass:"
            f"{str(audit_inlet_source_rms_k_surrogate_gate).strip().lower()}"
        )
    allowed_fidelity = {
...
def native_inlet_precondition_traceability_status(
    ...
    source_rms_k_surrogate_gate = str(
        get_any(native_preconditions_audit, ["inlet_source_rms_k_surrogate_gate"]) or ""
    ).strip().lower()
    ...
    for reason in as_string_list(
        get_any(
            native_preconditions_audit,
            ["inlet_source_rms_k_surrogate_reasons", "inlet_source_rms_k_surrogate_reasons_csv"],
        )
    ):
...
def build_report(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    audit_has_rms_k_velocity_surrogate = as_bool(
        get_any(inlet_source_audit, ["inlet_source_has_rms_k_velocity_surrogate"])
    )
    audit_inlet_source_rms_k_surrogate_gate = str(
        get_any(inlet_source_audit, ["inlet_source_rms_k_surrogate_gate"]) or ""
    ).strip().lower()
    ...
    inlet_source_evidence_ok = (
        ...
        and audit_has_rms_k_velocity_surrogate is not True
        and audit_inlet_source_rms_k_surrogate_gate == "pass"
    )
    add_gate(
        gates,
        ...
        (
            ...
            f"audit_has_rms_k_velocity_surrogate={audit_has_rms_k_velocity_surrogate}; "
            f"audit_inlet_source_rms_k_surrogate_gate={audit_inlet_source_rms_k_surrogate_gate or 'missing'}; "
