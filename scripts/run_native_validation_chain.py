...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--official-wind-filter", default="", help="Optional official RS wind-direction filter. Defaults to --wind-direction-label in scripts that support it.")
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional native_fluidx3d_baseline_manifest.json. "
            "Use this when --run-dir points directly to an output/VTK directory."
        ),
    )
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional runtime *_inlet_turbulence_stats.csv emitted by setup.cpp. "
            "Defaults to RuntimeInletDiagnosticsGate.CsvPath in --native-manifest when present."
        ),
    )
...
def native_baseline_gate_from_manifest(manifest: Dict[str, Any], manifest_path: Optional[Path]) -> str:
    ...
    for role in ["setup", "defines"]:
        ...
        if not json_bool(pair.get("Match")) and not json_bool(pair.get("AllowedMismatch")):
    ...
    runtime_inlet = manifest.get("RuntimeInletDiagnosticsGate")
    if isinstance(runtime_inlet, dict):
        runtime_inlet_requested = json_bool(runtime_inlet.get("Requested"))
        runtime_inlet_status = str(runtime_inlet.get("Gate") or "").strip().lower()
        if runtime_inlet_requested and runtime_inlet_status != "pass":
            return f"native_runtime_inlet_diagnostics_not_pass:{runtime_inlet_status or 'missing'}"
...
def resolve_optional_existing_path(raw: Any, base_path: Optional[Path]) -> Optional[Path]:
...
def discover_inlet_diagnostics_csv(
    explicit: str,
    native_manifest: Dict[str, Any],
    native_manifest_path: Optional[Path],
) -> Optional[Path]:
    explicit_path = resolve_optional_existing_path(explicit, None)
    if explicit_path is not None:
        return explicit_path
    gate = native_manifest.get("RuntimeInletDiagnosticsGate")
    if not isinstance(gate, dict):
        return None
    return resolve_optional_existing_path(gate.get("CsvPath"), native_manifest_path)
...
def main() -> int:
    ...
    inlet_diagnostics_csv = out_dir / "inlet_diagnostics_csv_summary.csv"
    ...
    native_manifest_path = (
        ...
        if args.native_manifest.strip()
        else find_run_file(run_dir, "native_fluidx3d_baseline_manifest.json")
    )
    if native_manifest_path is not None and not native_manifest_path.exists():
        raise SystemExit(f"native manifest does not exist: {native_manifest_path}")
    ...
    inlet_diagnostics_source = discover_inlet_diagnostics_csv(
    ...
    manifest: Dict[str, Any] = {
        ...
        "Inputs": {
            ...
            "InletDiagnosticsCsv": str(inlet_diagnostics_source) if inlet_diagnostics_source else "",
        },
        ...
        "Artifacts": {
            ...
            "InletDiagnosticsCsvAuditJson": str(inlet_diagnostics_json),
            "InletDiagnosticsCsvAuditCsv": str(inlet_diagnostics_csv),
    ...
    try:
        ...
        if setup_cpp:
            ...
            if defines_hpp:
                boundary_source_cmd.extend(["--defines", str(defines_hpp)])
        ...
        if inlet_diagnostics_source is not None:
            inlet_diagnostics_cmd = [
                py,
                str(script_dir / "audit_inlet_diagnostics_csv.py"),
                str(inlet_diagnostics_source),
                "--out-json",
            ...
            manifest["Steps"].append(run_step("audit_inlet_diagnostics_csv", inlet_diagnostics_cmd, allow_fail=True))
        else:
            inlet_diagnostics_json.write_text(
                json.dumps(
                    {
                        "Schema": "citylbm.inlet_diagnostics_csv_audit.v1",
                        "GeneratedAtUtc": utc_now(),
                        "Gate": "not_applicable",
                        "Reasons": ["inlet_diagnostics_csv_not_declared"],
                        "CsvPath": "",
                    },
                    indent=2,
                    ensure_ascii=True,
            ...
            manifest["Steps"].append(
                {
                    "Name": "audit_inlet_diagnostics_csv",
                    "Command": "",
                    "StartedAtUtc": utc_now(),
                    "FinishedAtUtc": utc_now(),
                    "ReturnCode": 0,
                    "Stdout": "",
                    "Stderr": "inlet diagnostics CSV not declared",
                    "AllowedToFail": True,
                }
            )
        write_manifest(manifest_path, manifest)
        ...
        domain_origin = run_dir / "domain_origin.json"
        if not domain_origin.exists():
            metadata_domain_origin = metadata.parent / "domain_origin.json"
            if metadata_domain_origin.exists():
                domain_origin = metadata_domain_origin
        if domain_origin.exists():
            probe_cmd.extend(["--domain-origin", str(domain_origin)])
