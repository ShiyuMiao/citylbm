...
namespace CityLBM.Solver
{
    ...
    public class FluidX3DInterface
    {
        ...
        private bool SupportsFluidX3DInletStressDdfReconstruction()
        {
            return FluidX3DSourceContains(
                "RECONSTRUCT_INLET_STRESS_DDF",
                "reconstruct_inlet_stress_boundaries");
        }

        private bool SupportsFluidX3DEquilibriumDdfReconstruction()
        {
            return FluidX3DSourceContains(
        ...
        private bool FluidX3DSourceContains(params string[] tokens)
        {
            if (string.IsNullOrWhiteSpace(FluidX3DPath) || tokens == null || tokens.Length == 0)
                return false;

            string srcDir = Path.Combine(FluidX3DPath, "src");
            string[] sourceFiles =
            {
                Path.Combine(srcDir, "lbm.hpp"),
                Path.Combine(srcDir, "lbm.cpp"),
                Path.Combine(srcDir, "kernel.cpp")
            };
            ...
            try
            {
                ...
                return tokens.All(token => combined.IndexOf(token, StringComparison.OrdinalIgnoreCase) >= 0);
            }
            catch
            {
                return false;
        ...
        private void GenerateDefinesHpp(Scene scene, CartesianGrid grid, SimulationSettings settings, string definesPath, bool enableGraphics = true)
        {
            ...
            bool supportsInletStressDdf = SupportsFluidX3DInletStressDdfReconstruction();
            bool supportsEquilibriumDdf = SupportsFluidX3DEquilibriumDdfReconstruction();
            sb.AppendLine("// ---- 扩展功能 ----");
            sb.AppendLine("#define FORCE_FIELD              // 允许体力");
            sb.AppendLine("#define EQUILIBRIUM_BOUNDARIES   // 平衡边界（TYPE_E，入口/出口需要）");
            if (syntheticInletActive && supportsInletStressDdf)
            {
                double zRefCells = scene.ReferenceHeight / Math.Max(grid.Dx, 1.0e-9);
                double inletStressMax = Math.Pow(TargetMaxProfileVelocityLbm * 0.5, 2.0);
                sb.AppendLine("#define RECONSTRUCT_INLET_STRESS_DDF  // CityLBM validation: rebuild inlet TYPE_E DDFs after STG refresh");
                sb.AppendLine($"#define INLET_STRESS_U_REF_LBM {TargetMaxProfileVelocityLbm.ToString("F8", CultureInfo.InvariantCulture)}f");
                sb.AppendLine($"#define INLET_STRESS_Z_REF_CELLS {zRefCells.ToString("F8", CultureInfo.InvariantCulture)}f");
                sb.AppendLine($"#define INLET_STRESS_POWER_ALPHA {scene.PowerLawAlpha.ToString("F8", CultureInfo.InvariantCulture)}f");
                sb.AppendLine("#define INLET_STRESS_GAIN 1.00000000f");
                sb.AppendLine($"#define INLET_STRESS_MAX_LBM2 {inletStressMax.ToString("F8", CultureInfo.InvariantCulture)}f");
            }
            else if (syntheticInletActive && supportsEquilibriumDdf)
            {
                sb.AppendLine("#define RECONSTRUCT_EQUILIBRIUM_BOUNDARY_DDF  // CityLBM validation: rebuild TYPE_E DDFs after STG refresh");
            }
            else if (syntheticInletActive)
            {
                sb.AppendLine("// RECONSTRUCT_INLET_STRESS_DDF / RECONSTRUCT_EQUILIBRIUM_BOUNDARY_DDF not enabled: FluidX3D source lacks the required LBM hooks.");
            }
            ...
            if (enableGraphics)
            {
                ...
                sb.AppendLine("#define GRAPHICS_LSF             4u");
                sb.AppendLine("#define GRAPHICS_LSQ             8u");
                sb.AppendLine("#define GRAPHICS_LSP             4u");
                                       ...
                                       string setupPath, string stlRelPath, string outputRelDir)
        {
            ...
            if (syntheticInletActive)
            {
                ...
                AppendSyntheticTurbulentInletApplyCode(sb, windDir, scene.Name, grid.Dx);
            }
            ...
            AppendEquivalentRoughWallDragInitialization(sb, scene, grid);
            AppendBoundaryProfileMaintenanceBufferInitialization(sb, scene.WindProfile, windDir);
            sb.AppendLine();
            sb.AppendLine("    lbm.flags.write_to_device();");
            sb.AppendLine("    lbm.u.write_to_device();");
            sb.AppendLine("    lbm.F.write_to_device();");
            sb.AppendLine();
            ...
            sb.AppendLine("    const bool empty_tunnel = false;  // Set true only for isolated inlet-preservation validation.");
            ...
            sb.AppendLine("    // ── 主模拟循环 ──");
            sb.AppendLine($"    const uint citylbm_total_steps = {settings.TimeSteps}u;");
            sb.AppendLine($"    const uint citylbm_save_interval = {settings.SaveInterval}u;");
            sb.AppendLine($"    const uint citylbm_vtk_save_start_step = {Math.Max(0, settings.VtkSaveStartStep)}u;");
            ...
            if (syntheticInletActive)
            {
                ...
                sb.AppendLine("    while(lbm.get_t() < citylbm_total_steps) {");
                sb.AppendLine("        uint remaining = citylbm_total_steps - (uint)lbm.get_t();");
            ...
            {
                sb.AppendLine("    lbm.run(citylbm_total_steps);  // 持续模拟直到 TimeSteps");
            }
            sb.AppendLine("#else // 非 GRAPHICS 模式：手动循环 + VTK 输出");
            sb.AppendLine($"    lbm.run(0u);  // 初始化（0步）");
            sb.AppendLine();
            sb.AppendLine("    uint citylbm_next_vtk_save_step = citylbm_vtk_save_start_step > 0u ? citylbm_vtk_save_start_step : citylbm_save_interval;");
            sb.AppendLine("    const bool citylbm_has_vtk_schedule = citylbm_next_vtk_save_step <= citylbm_total_steps;");
            sb.AppendLine("    while(lbm.get_t() < citylbm_total_steps) {");
            sb.AppendLine("        uint remaining = citylbm_total_steps - (uint)lbm.get_t();");
            sb.AppendLine("        uint steps_to_run = remaining < citylbm_save_interval ? remaining : citylbm_save_interval;");
            sb.AppendLine("        if(citylbm_has_vtk_schedule && citylbm_next_vtk_save_step > (uint)lbm.get_t()) {");
            sb.AppendLine("            uint until_next_save = citylbm_next_vtk_save_step - (uint)lbm.get_t();");
            ...
            sb.AppendLine("        lbm.run(steps_to_run);");
            sb.AppendLine();
            sb.AppendLine("        bool citylbm_should_save_vtk = citylbm_has_vtk_schedule &&");
            sb.AppendLine("            ((uint)lbm.get_t() >= citylbm_next_vtk_save_step || (uint)lbm.get_t() >= citylbm_total_steps);");
            sb.AppendLine("        if(citylbm_should_save_vtk) {");
            if (syntheticInletActive)
            {
                sb.AppendLine("            applySyntheticTurbulentInlet((uint)lbm.get_t());  // refresh saved VTK inlet boundary to the current CustomTable+k/Rij target.");
            }
            sb.AppendLine("            // 输出 VTK（速度场）到指定目录");
            sb.AppendLine($"            // path 只传目录前缀，default_filename() 会自动拼接 name-timestep.vtk");
            sb.AppendLine($"            lbm.u.write_device_to_vtk(\"{outputRelDir}/\", true);  // FluidX3D writes lattice velocity; metadata records VelocityScaleLbmToMps for SI post-processing.");
            sb.AppendLine("            while(citylbm_next_vtk_save_step <= (uint)lbm.get_t()) citylbm_next_vtk_save_step += citylbm_save_interval;");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        print_info(\"Step: \" + to_string(lbm.get_t()) +");
            sb.AppendLine("                   \" / \" + to_string(citylbm_total_steps));");
            sb.AppendLine("    }");
        ...
        private void AppendEquivalentRoughWallDragInitialization(StringBuilder sb, Scene scene, CartesianGrid grid)
        {
            double dx = Math.Max(grid.Dx, 1.0e-9);
            double roughnessLength = Math.Max(scene.RoughnessLength, 1.0e-6);
            double kappa = Math.Max(scene.VonKarmanConstant, 0.1);
            double roughnessLengthCells = roughnessLength / dx;
            double roughWallLayerCells = Math.Max(3.0, Math.Ceiling(Math.Max(2.0 * dx, 10.0 * roughnessLength) / dx));
            const double maxVelocityFractionPerStep = 0.02;

            sb.AppendLine();
            sb.AppendLine("    // CityLBM v0.3.0 validation fix: equivalent rough-wall drag in the near-ground layer.");
            sb.AppendLine("    // This writes a real FluidX3D FORCE_FIELD, not a post-processing visualization overlay.");
            sb.AppendLine("    // The drag is derived from a log-law friction velocity using RoughnessLength and is capped for LBM stability.");
            sb.AppendLine($"    const float roughness_length_cells = {roughnessLengthCells.ToString("F8", CultureInfo.InvariantCulture)}f;");
            sb.AppendLine($"    const float rough_wall_layer_cells = {roughWallLayerCells.ToString("F8", CultureInfo.InvariantCulture)}f;");
            sb.AppendLine($"    const float rough_wall_kappa = {kappa.ToString("F8", CultureInfo.InvariantCulture)}f;");
            sb.AppendLine($"    const float rough_wall_max_velocity_fraction_per_step = {maxVelocityFractionPerStep.ToString("F8", CultureInfo.InvariantCulture)}f;");
            sb.AppendLine("    auto rough_wall_function = [&](float speed_lbm, float z_cell_center) {");
            sb.AppendLine("        float z0 = fmax(roughness_length_cells, 1.0e-6f);");
            sb.AppendLine("        float denom = log(fmax((z_cell_center + z0) / z0, 1.0001f));");
            sb.AppendLine("        if(speed_lbm <= 0.0f || denom <= 1.0e-5f) return 0.0f;");
            sb.AppendLine("        float friction_velocity = rough_wall_kappa * speed_lbm / denom;");
            sb.AppendLine("        return friction_velocity * friction_velocity / fmax(rough_wall_layer_cells, 1.0f);");
            sb.AppendLine("    };");
            sb.AppendLine("    auto rough_wall_drag_limit = [&](float rough_wall_drag, float speed_lbm) {");
            sb.AppendLine("        return fmin(rough_wall_drag, rough_wall_max_velocity_fraction_per_step * speed_lbm);");
            sb.AppendLine("    };");
            sb.AppendLine("    auto apply_rough_wall = [&]() {");
            sb.AppendLine("        parallel_for(lbm.get_N(), [&](ulong n) {");
            sb.AppendLine("            uint x=0u, y=0u, z=0u;");
            sb.AppendLine("            lbm.coordinates(n, x, y, z);");
            sb.AppendLine("            if(z == 0u || (float)z > rough_wall_layer_cells) return;");
            sb.AppendLine("            if((lbm.flags[n]&TYPE_S)==TYPE_S || (lbm.flags[n]&TYPE_E)==TYPE_E) return;");
            sb.AppendLine("            float ux = lbm.u.x[n];");
            sb.AppendLine("            float uy = lbm.u.y[n];");
            sb.AppendLine("            float horizontal_speed = sqrt(ux*ux + uy*uy);");
            sb.AppendLine("            if(horizontal_speed <= 1.0e-8f) return;");
            sb.AppendLine("            float z_cell_center = fmax((float)z + 0.5f, roughness_length_cells * 1.01f);");
            sb.AppendLine("            float wall_weight = fmax(0.0f, 1.0f - ((float)z / fmax(rough_wall_layer_cells, 1.0f)));");
            sb.AppendLine("            float rough_wall_drag = rough_wall_drag_limit(rough_wall_function(horizontal_speed, z_cell_center) * wall_weight, horizontal_speed);");
            sb.AppendLine("            lbm.F.x[n] = -rough_wall_drag * ux / horizontal_speed;");
            sb.AppendLine("            lbm.F.y[n] = -rough_wall_drag * uy / horizontal_speed;");
            sb.AppendLine("            lbm.F.z[n] = 0.0f;");
            sb.AppendLine("        });");
            sb.AppendLine("    };");
            sb.AppendLine("    apply_rough_wall();");
        }

        private void AppendBoundaryProfileMaintenanceBufferInitialization(StringBuilder sb, WindProfileType windProfile, Vector3d windDir)
        {
            bool xDominant = Math.Abs(windDir.X) >= Math.Abs(windDir.Y);
            bool flowFromMin = xDominant ? windDir.X > 0.0 : windDir.Y > 0.0;

            sb.AppendLine();
            sb.AppendLine("    // CityLBM v0.3.0 validation fix: profile-maintenance FORCE_FIELD buffer near open boundaries.");
            sb.AppendLine("    // This writes a real FluidX3D body-force buffer at outlet/lateral/top interior cells.");
            sb.AppendLine("    // It reduces zero-gradient/open-box profile collapse in short urban validation domains,");
            sb.AppendLine("    // but it is still diagnostic and does not replace a validated non-reflecting outlet or precursor/recycling domain.");
            sb.AppendLine("    const float boundary_profile_maintenance_layer_cells = 8.0f;");
            sb.AppendLine("    const float boundary_profile_maintenance_gain = 0.00150000f;");
            sb.AppendLine("    const float boundary_profile_maintenance_max_force = 0.00030000f;");
            if (windProfile == WindProfileType.Uniform)
            {
                sb.AppendLine("    auto boundary_profile_target = [&](uint z_cell) -> float3 {");
                sb.AppendLine("        (void)z_cell;");
            ...
            {
                sb.AppendLine("    auto boundary_profile_target = [&](uint z_cell) -> float3 {");
                sb.AppendLine("        return windProfile(z_cell);");
                sb.AppendLine("    };");
            }
            sb.AppendLine("    auto boundary_profile_buffer_weight = [&](float distance_to_boundary) {");
            sb.AppendLine("        if(distance_to_boundary < 1.0f || distance_to_boundary >= boundary_profile_maintenance_layer_cells) return 0.0f;");
            sb.AppendLine("        float eta = 1.0f - distance_to_boundary / boundary_profile_maintenance_layer_cells;");
            sb.AppendLine("        return eta * eta;");
            sb.AppendLine("    };");
            sb.AppendLine("    auto apply_boundary_profile_maintenance_buffer = [&]() {");
            sb.AppendLine("        parallel_for(lbm.get_N(), [&](ulong n) {");
            sb.AppendLine("            uint x=0u, y=0u, z=0u;");
            sb.AppendLine("            lbm.coordinates(n, x, y, z);");
            sb.AppendLine("            if((lbm.flags[n]&TYPE_S)==TYPE_S || (lbm.flags[n]&TYPE_E)==TYPE_E) return;");
            sb.AppendLine($"            const bool citylbm_flow_x_dominant = {(xDominant ? "true" : "false")};");
            sb.AppendLine($"            const bool citylbm_flow_from_min = {(flowFromMin ? "true" : "false")};");
            sb.AppendLine("            float outlet_distance = citylbm_flow_x_dominant");
            sb.AppendLine("                ? (citylbm_flow_from_min ? (float)(Nx-1u-x) : (float)x)");
            sb.AppendLine("                : (citylbm_flow_from_min ? (float)(Ny-1u-y) : (float)y);");
            sb.AppendLine("            float lateral_a_distance = citylbm_flow_x_dominant ? (float)y : (float)x;");
            sb.AppendLine("            float lateral_b_distance = citylbm_flow_x_dominant ? (float)(Ny-1u-y) : (float)(Nx-1u-x);");
            sb.AppendLine("            float top_distance = (float)(Nz-1u-z);");
            sb.AppendLine("            float outlet_weight = boundary_profile_buffer_weight(outlet_distance);");
            sb.AppendLine("            float lateral_weight = fmax(boundary_profile_buffer_weight(lateral_a_distance), boundary_profile_buffer_weight(lateral_b_distance));");
            sb.AppendLine("            float top_weight = boundary_profile_buffer_weight(top_distance);");
            sb.AppendLine("            float buffer_weight = fmax(outlet_weight, fmax(lateral_weight, top_weight));");
            sb.AppendLine("            if(buffer_weight <= 0.0f) return;");
            sb.AppendLine("            float3 target = boundary_profile_target(z);");
            sb.AppendLine("            float target_speed = sqrt(target.x*target.x + target.y*target.y + target.z*target.z);");
            sb.AppendLine("            if(target_speed <= 1.0e-8f) return;");
            sb.AppendLine("            float force_scale = fmin(boundary_profile_maintenance_gain * buffer_weight, boundary_profile_maintenance_max_force / target_speed);");
            sb.AppendLine("            lbm.F.x[n] += force_scale * target.x;");
            sb.AppendLine("            lbm.F.y[n] += force_scale * target.y;");
            sb.AppendLine("            lbm.F.z[n] += force_scale * target.z;");
            sb.AppendLine("        });");
            sb.AppendLine("    };");
            sb.AppendLine("    apply_boundary_profile_maintenance_buffer();");
        }
        ...
        private void AppendCustomTableProfileCode(StringBuilder sb, Scene scene, double dx, double originZ, double uScale, Vector3d windDir)
        {
            ...
            bool hasMeasuredDiagonalRms = HasCompleteCustomRmsProfile(scene);
            bool hasFullReynoldsStressTensor = HasCompleteCustomReynoldsStressTensorProfile(scene);
            ...
            sb.AppendLine("    const bool profile_has_measured_diagonal_rms = " + (hasMeasuredDiagonalRms ? "true" : "false") + ";");
            sb.AppendLine("    const bool profile_has_full_reynolds_stress_tensor = " + (hasFullReynoldsStressTensor ? "true" : "false") + ";");
            sb.AppendLine("    const float profile_u_rms_lbm[profile_count] = {" + JoinFloatArray(samples.Select(s => hasMeasuredDiagonalRms ? Math.Max(0.0, s.URms) * uScale : 0.0)) + "};");
            sb.AppendLine("    const float profile_v_rms_lbm[profile_count] = {" + JoinFloatArray(samples.Select(s => hasMeasuredDiagonalRms ? Math.Max(0.0, s.VRms) * uScale : 0.0)) + "};");
            sb.AppendLine("    const float profile_w_rms_lbm[profile_count] = {" + JoinFloatArray(samples.Select(s => hasMeasuredDiagonalRms ? Math.Max(0.0, s.WRms) * uScale : 0.0)) + "};");
            sb.AppendLine(hasFullReynoldsStressTensor
                ? "    // Reynolds-stress arrays use measured/precursor full tensor columns: R11, R22, R33, R12, R13 and R23."
                : (hasMeasuredDiagonalRms
                    ? "    // Reynolds-stress diagonal arrays use measured RMS columns: R11=u_rms^2, R22=v_rms^2, R33=w_rms^2. Off-diagonal covariances are unavailable and remain zero for diagnostics."
                    : "    // Reynolds-stress arrays are derived from the isotropic-k assumption: R11=R22=R33=2k/3 and R12=R13=R23=0."));
            sb.AppendLine("    // These arrays are inlet-source traceability data and STG-lite targets; paper-grade use still requires native U/k/correlation preservation gates.");
            sb.AppendLine("    const float profile_r11_lbm[profile_count] = {" + JoinFloatArray(samples.Select(s => hasFullReynoldsStressTensor ? Math.Max(0.0, s.R11) * uScale * uScale : (hasMeasuredDiagonalRms ? Math.Max(0.0, s.URms) * Math.Max(0.0, s.URms) * uScale * uScale : (s.HasK ? (2.0 * Math.Max(0.0, s.K) / 3.0) * uScale * uScale : 0.0)))) + "};");
            sb.AppendLine("    const float profile_r22_lbm[profile_count] = {" + JoinFloatArray(samples.Select(s => hasFullReynoldsStressTensor ? Math.Max(0.0, s.R22) * uScale * uScale : (hasMeasuredDiagonalRms ? Math.Max(0.0, s.VRms) * Math.Max(0.0, s.VRms) * uScale * uScale : (s.HasK ? (2.0 * Math.Max(0.0, s.K) / 3.0) * uScale * uScale : 0.0)))) + "};");
            sb.AppendLine("    const float profile_r33_lbm[profile_count] = {" + JoinFloatArray(samples.Select(s => hasFullReynoldsStressTensor ? Math.Max(0.0, s.R33) * uScale * uScale : (hasMeasuredDiagonalRms ? Math.Max(0.0, s.WRms) * Math.Max(0.0, s.WRms) * uScale * uScale : (s.HasK ? (2.0 * Math.Max(0.0, s.K) / 3.0) * uScale * uScale : 0.0)))) + "};");
            sb.AppendLine("    const float profile_r12_lbm[profile_count] = {" + JoinFloatArray(samples.Select(s => hasFullReynoldsStressTensor ? s.R12 * uScale * uScale : 0.0)) + "};");
            sb.AppendLine("    const float profile_r13_lbm[profile_count] = {" + JoinFloatArray(samples.Select(s => hasFullReynoldsStressTensor ? s.R13 * uScale * uScale : 0.0)) + "};");
            sb.AppendLine("    const float profile_r23_lbm[profile_count] = {" + JoinFloatArray(samples.Select(s => hasFullReynoldsStressTensor ? s.R23 * uScale * uScale : 0.0)) + "};");
            ...
            sb.AppendLine("    };");
            sb.AppendLine("    auto interpolate_profile_value = [&](const float* values, float z_m) -> float {");
            sb.AppendLine("        if(z_m <= profile_z_m[0]) return values[0];");
            sb.AppendLine("        if(z_m >= profile_z_m[profile_count-1]) return values[profile_count-1];");
            sb.AppendLine("        for(int i=0; i<profile_count-1; i++) {");
            sb.AppendLine("            if(z_m >= profile_z_m[i] && z_m <= profile_z_m[i+1]) {");
            sb.AppendLine("                float dz = profile_z_m[i+1] - profile_z_m[i];");
            sb.AppendLine("                if(dz < 1.0e-6f) dz = 1.0e-6f;");
            sb.AppendLine("                float t = (z_m - profile_z_m[i]) / dz;");
            sb.AppendLine("                return values[i] + t * (values[i+1] - values[i]);");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine("        return values[profile_count-1];");
            sb.AppendLine("    };");
            sb.AppendLine("    struct CityLBMReynoldsCholesky { float l11; float l21; float l22; float l31; float l32; float l33; };");
            sb.AppendLine("    auto citylbm_stg_target_reynolds_cholesky = [&](float z_m) -> CityLBMReynoldsCholesky {");
            sb.AppendLine("        float r11 = fmaxf(0.0f, interpolate_profile_value(profile_r11_lbm, z_m));");
            sb.AppendLine("        float r22 = fmaxf(0.0f, interpolate_profile_value(profile_r22_lbm, z_m));");
            sb.AppendLine("        float r33 = fmaxf(0.0f, interpolate_profile_value(profile_r33_lbm, z_m));");
            sb.AppendLine("        float r12 = interpolate_profile_value(profile_r12_lbm, z_m);");
            sb.AppendLine("        float r13 = interpolate_profile_value(profile_r13_lbm, z_m);");
            sb.AppendLine("        float r23 = interpolate_profile_value(profile_r23_lbm, z_m);");
            sb.AppendLine("        CityLBMReynoldsCholesky l;");
            sb.AppendLine("        l.l11 = sqrtf(r11);");
            sb.AppendLine("        l.l21 = l.l11 > 1.0e-12f ? r12 / l.l11 : 0.0f;");
            sb.AppendLine("        l.l31 = l.l11 > 1.0e-12f ? r13 / l.l11 : 0.0f;");
            sb.AppendLine("        l.l22 = sqrtf(fmaxf(0.0f, r22 - l.l21*l.l21));");
            sb.AppendLine("        l.l32 = l.l22 > 1.0e-12f ? (r23 - l.l31*l.l21) / l.l22 : 0.0f;");
            sb.AppendLine("        l.l33 = sqrtf(fmaxf(0.0f, r33 - l.l31*l.l31 - l.l32*l.l32));");
        ...
        private bool HasCompleteCustomRmsProfile(Scene scene)
        {
            return scene.CustomWindProfile != null &&
                   scene.CustomWindProfile.Count >= 2 &&
                   scene.CustomWindProfile.All(s => s.HasRms);
        }

        private bool HasCompleteCustomReynoldsStressTensorProfile(Scene scene)
        {
            return scene.CustomWindProfile != null &&
                   scene.CustomWindProfile.Count >= 2 &&
                   scene.CustomWindProfile.All(s => s.HasReynoldsStressTensor);
        }
        ...
        private void AppendSyntheticTurbulentInletVelocityCode(StringBuilder sb, SimulationSettings settings, double dx)
        {
            ...
            sb.AppendLine("    // Temporal evolution uses continuous-step Taylor frozen-turbulence advection along the local mean LBM wind and a persistent synthetic-eddy population.");
            ...
            sb.AppendLine("    // A light deterministic phase blend reduces update-to-update jumps without quantizing the saved-frame turbulence phase.");
            sb.AppendLine("    // This is a diagnostic approximation, not a full digital-filter/SEM/precursor inlet with measured Reynolds-stress tensors.");
            sb.AppendLine("    // When the selected FluidX3D source exposes reconstruction hooks, applyInlet refreshes macroscopic inlet velocity fields and rebuilds TYPE_E DDFs before the next run chunk.");
            ...
            sb.AppendLine("    const float citylbm_stg_temporal_ar1_rho = 0.650000f;");
            ...
            sb.AppendLine("    const float citylbm_stg_temporal_step_scale = 0.250000f;");
            ...
            sb.AppendLine("    bool citylbm_sem_eddy_initialized = false;");
            ...
            sb.AppendLine("        float phase = 0.03125f * (float)t_step * citylbm_stg_temporal_step_scale;");
            sb.AppendLine("        temporal_filter_state_x = citylbm_stg_temporal_ar1_rho * sinf(phase + 0.17f) + 0.25f * citylbm_stg_temporal_ar1_innovation_scale * sinf(0.37f * phase + 2.11f);");
            sb.AppendLine("        temporal_filter_state_y = citylbm_stg_temporal_ar1_rho * sinf(phase + 1.91f) + 0.25f * citylbm_stg_temporal_ar1_innovation_scale * sinf(0.41f * phase + 3.29f);");
            sb.AppendLine("        temporal_filter_state_z = citylbm_stg_temporal_ar1_rho * sinf(phase + 3.73f) + 0.25f * citylbm_stg_temporal_ar1_innovation_scale * sinf(0.43f * phase + 4.07f);");
            sb.AppendLine("    };");
            sb.AppendLine("    auto wrapSyntheticEddyPopulation = [&](int m) {");
            sb.AppendLine("        while(sem_eddy[m].eddy_center_x < 0.0f) sem_eddy[m].eddy_center_x += (float)Nx;");
            sb.AppendLine("        while(sem_eddy[m].eddy_center_x >= (float)Nx) sem_eddy[m].eddy_center_x -= (float)Nx;");
            sb.AppendLine("        while(sem_eddy[m].eddy_center_y < 0.0f) sem_eddy[m].eddy_center_y += (float)Ny;");
            sb.AppendLine("        while(sem_eddy[m].eddy_center_y >= (float)Ny) sem_eddy[m].eddy_center_y -= (float)Ny;");
            sb.AppendLine("        while(sem_eddy[m].eddy_center_z < 0.0f) sem_eddy[m].eddy_center_z += (float)Nz;");
            sb.AppendLine("        while(sem_eddy[m].eddy_center_z >= (float)Nz) sem_eddy[m].eddy_center_z -= (float)Nz;");
            sb.AppendLine("    };");
            sb.AppendLine("    auto initializeSyntheticEddyPopulation = [&]() {");
            sb.AppendLine("        for(int m=0; m<synthetic_eddy_count; m++) {");
            sb.AppendLine("            uint seed = (uint)(m + 1) * 747796405u;");
            sb.AppendLine("            sem_eddy[m].eddy_center_x = hash01(seed + 11u) * (float)Nx;");
            sb.AppendLine("            sem_eddy[m].eddy_center_y = hash01(seed + 23u) * (float)Ny;");
            sb.AppendLine("            sem_eddy[m].eddy_center_z = hash01(seed + 37u) * (float)Nz;");
            ...
            sb.AppendLine("        }");
            sb.AppendLine("        citylbm_sem_eddy_initialized = true;");
            sb.AppendLine("    };");
            sb.AppendLine("    auto updateSyntheticEddyPlane = [&](uint t_step) {");
            sb.AppendLine("        updateTemporalFilter(t_step);");
            sb.AppendLine("        if(!citylbm_sem_eddy_initialized) { initializeSyntheticEddyPopulation(); return; }");
            sb.AppendLine("        uint delta_t = (citylbm_sem_last_update_t_step == 4294967295u || t_step < citylbm_sem_last_update_t_step) ? 0u : (t_step - citylbm_sem_last_update_t_step);");
            sb.AppendLine("        float advect_steps = (float)delta_t * 0.10f * citylbm_stg_temporal_step_scale;");
            sb.AppendLine("        float advect = advect_steps;");
            sb.AppendLine("        for(int m=0; m<synthetic_eddy_count; m++) {");
            sb.AppendLine("            sem_eddy[m].eddy_center_x += dir_x * advect;");
            sb.AppendLine("            sem_eddy[m].eddy_center_y += dir_y * advect;");
            sb.AppendLine("            sem_eddy[m].eddy_center_z += dir_z * advect;");
            sb.AppendLine("            wrapSyntheticEddyPopulation(m);");
            sb.AppendLine("        }");
            sb.AppendLine("    };");
            ...
            sb.AppendLine("    };");
            sb.AppendLine("    // Target component RMS uses measured diagonal RMS when provided; otherwise it falls back to isotropic k: sigma=sqrt(2k/3).");
            ...
            sb.AppendLine("        return direction * (float)h * 0.25f / citylbm_stg_corr_cells;");
            sb.AppendLine("    };");
            ...
            sb.AppendLine("    };");
            sb.AppendLine("    auto citylbm_stg_target_sigma_xyz = [&](uint z_cell, float3 mean) -> float3 {");
            sb.AppendLine($"        float z_m = profile_origin_z_m + ((float)z_cell + 0.5f) * {dx.ToString("F8", CultureInfo.InvariantCulture)}f;");
            sb.AppendLine("        float k_lbm = interpolate_profile_k(z_m);");
            sb.AppendLine("        if(k_lbm < 0.0f) k_lbm = 0.0f;");
            sb.AppendLine("        float isotropic_sigma = sqrtf(0.6666667f * k_lbm);");
            sb.AppendLine("        float sigma_x = profile_has_full_reynolds_stress_tensor ? sqrtf(fmaxf(0.0f, interpolate_profile_value(profile_r11_lbm, z_m))) : (profile_has_measured_diagonal_rms ? interpolate_profile_value(profile_u_rms_lbm, z_m) : isotropic_sigma);");
            sb.AppendLine("        float sigma_y = profile_has_full_reynolds_stress_tensor ? sqrtf(fmaxf(0.0f, interpolate_profile_value(profile_r22_lbm, z_m))) : (profile_has_measured_diagonal_rms ? interpolate_profile_value(profile_v_rms_lbm, z_m) : isotropic_sigma);");
            sb.AppendLine("        float sigma_z = profile_has_full_reynolds_stress_tensor ? sqrtf(fmaxf(0.0f, interpolate_profile_value(profile_r33_lbm, z_m))) : (profile_has_measured_diagonal_rms ? interpolate_profile_value(profile_w_rms_lbm, z_m) : isotropic_sigma);");
            sb.AppendLine("        sigma_x *= citylbm_stg_scale; sigma_y *= citylbm_stg_scale; sigma_z *= citylbm_stg_scale;");
            ...
            sb.AppendLine("        if(sigma_x > cap) sigma_x = cap; if(sigma_y > cap) sigma_y = cap; if(sigma_z > cap) sigma_z = cap;");
            sb.AppendLine("        return float3(sigma_x, sigma_y, sigma_z);");
            sb.AppendLine("    };");
            ...
            sb.AppendLine("        float3 sigma = citylbm_stg_target_sigma_xyz(z_cell, mean);");
            sb.AppendLine("        float mean_mag = sqrtf(mean.x*mean.x + mean.y*mean.y + mean.z*mean.z);");
            sb.AppendLine("        float citylbm_stg_advect_steps = (float)t_step * citylbm_stg_temporal_step_scale;");
            sb.AppendLine("        float citylbm_stg_prev_t_step = t_step > citylbm_stg_update_interval ? (float)(t_step - citylbm_stg_update_interval) : 0.0f;");
            sb.AppendLine("        float citylbm_stg_prev_advect_steps = citylbm_stg_prev_t_step * citylbm_stg_temporal_step_scale;");
            sb.AppendLine("        float citylbm_stg_temporal_rho = t_step > citylbm_stg_update_interval ? citylbm_stg_temporal_ar1_rho : 0.0f;");
            sb.AppendLine("        float citylbm_stg_temporal_innovation = t_step > citylbm_stg_update_interval ? citylbm_stg_temporal_ar1_innovation_scale : 1.0f;");
            sb.AppendLine("        float advected_x = (float)x - dir_x * mean_mag * citylbm_stg_advect_steps;");
            sb.AppendLine("        float advected_y = (float)y - dir_y * mean_mag * citylbm_stg_advect_steps;");
            sb.AppendLine("        float advected_z = (float)z_cell - dir_z * mean_mag * citylbm_stg_advect_steps;");
            sb.AppendLine("        float prev_advected_x = (float)x - dir_x * mean_mag * citylbm_stg_prev_advect_steps;");
            sb.AppendLine("        float prev_advected_y = (float)y - dir_y * mean_mag * citylbm_stg_prev_advect_steps;");
            sb.AppendLine("        float prev_advected_z = (float)z_cell - dir_z * mean_mag * citylbm_stg_prev_advect_steps;");
            sb.AppendLine("        float fluct_x = 0.0f, fluct_y = 0.0f, fluct_z = 0.0f;");
            ...
            sb.AppendLine("            float previous_phase = kx * prev_advected_x + ky * prev_advected_y + kz * prev_advected_z;");
            sb.AppendLine("            float wave_x = citylbm_stg_temporal_rho * sinf(previous_phase + citylbm_mode_phase(m, 0)) + citylbm_stg_temporal_innovation * sinf(phase + citylbm_mode_phase(m, 0));");
            sb.AppendLine("            float wave_y = citylbm_stg_temporal_rho * sinf(previous_phase + citylbm_mode_phase(m, 1)) + citylbm_stg_temporal_innovation * sinf(phase + citylbm_mode_phase(m, 1));");
            sb.AppendLine("            float wave_z = citylbm_stg_temporal_rho * sinf(previous_phase + citylbm_mode_phase(m, 2)) + citylbm_stg_temporal_innovation * sinf(phase + citylbm_mode_phase(m, 2));");
            ...
            sb.AppendLine("        fluct_x = 0.90f * fluct_x + 0.10f * sem_fluct.x;");
            sb.AppendLine("        fluct_y = 0.90f * fluct_y + 0.10f * sem_fluct.y;");
            sb.AppendLine("        fluct_z = 0.90f * fluct_z + 0.10f * sem_fluct.z;");
            sb.AppendLine("        float3 perturbation = float3(sigma.x * fluct_x, sigma.y * fluct_y, sigma.z * fluct_z);");
            sb.AppendLine("        if(profile_has_full_reynolds_stress_tensor) {");
            sb.AppendLine($"            float z_m = profile_origin_z_m + ((float)z_cell + 0.5f) * {dx.ToString("F8", CultureInfo.InvariantCulture)}f;");
            sb.AppendLine("            CityLBMReynoldsCholesky l = citylbm_stg_target_reynolds_cholesky(z_m);");
            sb.AppendLine("            perturbation = float3(l.l11 * fluct_x, l.l21 * fluct_x + l.l22 * fluct_y, l.l31 * fluct_x + l.l32 * fluct_y + l.l33 * fluct_z);");
            sb.AppendLine("            perturbation.x *= citylbm_stg_scale; perturbation.y *= citylbm_stg_scale; perturbation.z *= citylbm_stg_scale;");
            sb.AppendLine("        }");
            sb.AppendLine("        float3 u = float3(mean.x + perturbation.x, mean.y + perturbation.y, mean.z + perturbation.z);");
        ...
        private void AppendSyntheticTurbulentInletApplyCode(StringBuilder sb, Vector3d windDir, string sceneName, double dx)
        {
            string inletCondition = GetInletFaceCondition(windDir);
            string diagnosticsCsv = SanitizeName(sceneName ?? "case").ToLowerInvariant() + "_inlet_turbulence_stats.csv";

            sb.AppendLine("    auto applyInlet = [&](uint t_step) {");
            sb.AppendLine("        // Distribution-reconstructed TYPE_E inlet refresh for diagnostic turbulent-inlet runs.");
            sb.AppendLine("        // Three-pass layer correction keeps finite-mode STG-lite / synthetic-eddy perturbations mean-preserving at every inlet z_cell.");
            sb.AppendLine("        // Full-tensor runs use layerwise covariance whitening/re-coloring so finite inlet planes preserve R11/R22/R33 and R12/R13/R23.");
            ...
            sb.AppendLine("        std::vector<float> citylbm_stg_layer_corrected_sum_xy(Nz, 0.0f);");
            sb.AppendLine("        std::vector<float> citylbm_stg_layer_corrected_sum_xz(Nz, 0.0f);");
            sb.AppendLine("        std::vector<float> citylbm_stg_layer_corrected_sum_yz(Nz, 0.0f);");
            ...
            sb.AppendLine("        std::vector<float> citylbm_stg_layer_source_l11(Nz, 1.0f);");
            sb.AppendLine("        std::vector<float> citylbm_stg_layer_source_l21(Nz, 0.0f);");
            sb.AppendLine("        std::vector<float> citylbm_stg_layer_source_l22(Nz, 1.0f);");
            sb.AppendLine("        std::vector<float> citylbm_stg_layer_source_l31(Nz, 0.0f);");
            sb.AppendLine("        std::vector<float> citylbm_stg_layer_source_l32(Nz, 0.0f);");
            sb.AppendLine("        std::vector<float> citylbm_stg_layer_source_l33(Nz, 1.0f);");
            sb.AppendLine("        std::vector<int> citylbm_stg_layer_tensor_whitening_valid(Nz, 0);");
            ...
            sb.AppendLine("                citylbm_stg_layer_corrected_sum_xy[z] += fluct_x * fluct_y;");
            sb.AppendLine("                citylbm_stg_layer_corrected_sum_xz[z] += fluct_x * fluct_z;");
            sb.AppendLine("                citylbm_stg_layer_corrected_sum_yz[z] += fluct_y * fluct_z;");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine("        for(uint z_layer=0u; z_layer<Nz; z_layer++) {");
            sb.AppendLine("            if(citylbm_stg_layer_corrected_inlet_count[z_layer] > 1ull) {");
            sb.AppendLine("                float inv_count = 1.0f / (float)citylbm_stg_layer_corrected_inlet_count[z_layer];");
            sb.AppendLine("                float3 target_sigma = citylbm_stg_target_sigma_xyz(z_layer, windProfile(z_layer));");
            ...
            sb.AppendLine("                citylbm_stg_layer_rms_scale_x[z_layer] = rms_x > 1.0e-12f ? target_sigma.x / rms_x : 1.0f;");
            sb.AppendLine("                citylbm_stg_layer_rms_scale_y[z_layer] = rms_y > 1.0e-12f ? target_sigma.y / rms_y : 1.0f;");
            sb.AppendLine("                citylbm_stg_layer_rms_scale_z[z_layer] = rms_z > 1.0e-12f ? target_sigma.z / rms_z : 1.0f;");
            sb.AppendLine("                if(profile_has_full_reynolds_stress_tensor) {");
            sb.AppendLine("                    float c11 = citylbm_stg_layer_corrected_sum_sq_x[z_layer] * inv_count;");
            sb.AppendLine("                    float c22 = citylbm_stg_layer_corrected_sum_sq_y[z_layer] * inv_count;");
            sb.AppendLine("                    float c33 = citylbm_stg_layer_corrected_sum_sq_z[z_layer] * inv_count;");
            sb.AppendLine("                    float c12 = citylbm_stg_layer_corrected_sum_xy[z_layer] * inv_count;");
            sb.AppendLine("                    float c13 = citylbm_stg_layer_corrected_sum_xz[z_layer] * inv_count;");
            sb.AppendLine("                    float c23 = citylbm_stg_layer_corrected_sum_yz[z_layer] * inv_count;");
            sb.AppendLine("                    float s11 = sqrtf(fmaxf(c11, 0.0f));");
            sb.AppendLine("                    float s21 = s11 > 1.0e-12f ? c12 / s11 : 0.0f;");
            sb.AppendLine("                    float s31 = s11 > 1.0e-12f ? c13 / s11 : 0.0f;");
            sb.AppendLine("                    float s22 = sqrtf(fmaxf(0.0f, c22 - s21*s21));");
            sb.AppendLine("                    float s32 = s22 > 1.0e-12f ? (c23 - s31*s21) / s22 : 0.0f;");
            sb.AppendLine("                    float s33 = sqrtf(fmaxf(0.0f, c33 - s31*s31 - s32*s32));");
            sb.AppendLine("                    if(s11 > 1.0e-12f && s22 > 1.0e-12f && s33 > 1.0e-12f) {");
            sb.AppendLine("                        citylbm_stg_layer_source_l11[z_layer] = s11;");
            sb.AppendLine("                        citylbm_stg_layer_source_l21[z_layer] = s21;");
            sb.AppendLine("                        citylbm_stg_layer_source_l22[z_layer] = s22;");
            sb.AppendLine("                        citylbm_stg_layer_source_l31[z_layer] = s31;");
            sb.AppendLine("                        citylbm_stg_layer_source_l32[z_layer] = s32;");
            sb.AppendLine("                        citylbm_stg_layer_source_l33[z_layer] = s33;");
            sb.AppendLine("                        citylbm_stg_layer_tensor_whitening_valid[z_layer] = 1;");
            sb.AppendLine("                    }");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            ...
            sb.AppendLine("                float fluct_y = u_in.y - mean.y - citylbm_stg_layer_mean_correction_y[z];");
            sb.AppendLine("                float fluct_z = u_in.z - mean.z - citylbm_stg_layer_mean_correction_z[z];");
            sb.AppendLine("                if(profile_has_full_reynolds_stress_tensor && citylbm_stg_layer_tensor_whitening_valid[z] == 1) {");
            sb.AppendLine("                    float w1 = fluct_x / citylbm_stg_layer_source_l11[z];");
            sb.AppendLine("                    float w2 = (fluct_y - citylbm_stg_layer_source_l21[z] * w1) / citylbm_stg_layer_source_l22[z];");
            sb.AppendLine("                    float w3 = (fluct_z - citylbm_stg_layer_source_l31[z] * w1 - citylbm_stg_layer_source_l32[z] * w2) / citylbm_stg_layer_source_l33[z];");
            sb.AppendLine($"                    float z_m = profile_origin_z_m + ((float)z + 0.5f) * {dx.ToString("F8", CultureInfo.InvariantCulture)}f;");
            sb.AppendLine("                    CityLBMReynoldsCholesky target_l = citylbm_stg_target_reynolds_cholesky(z_m);");
            sb.AppendLine("                    u_in.x = mean.x + citylbm_stg_scale * target_l.l11 * w1;");
            sb.AppendLine("                    u_in.y = mean.y + citylbm_stg_scale * (target_l.l21 * w1 + target_l.l22 * w2);");
            sb.AppendLine("                    u_in.z = mean.z + citylbm_stg_scale * (target_l.l31 * w1 + target_l.l32 * w2 + target_l.l33 * w3);");
            sb.AppendLine("                } else {");
            sb.AppendLine("                    u_in.x = mean.x + fluct_x * citylbm_stg_layer_rms_scale_x[z];");
            sb.AppendLine("                    u_in.y = mean.y + fluct_y * citylbm_stg_layer_rms_scale_y[z];");
            sb.AppendLine("                    u_in.z = mean.z + fluct_z * citylbm_stg_layer_rms_scale_z[z];");
            sb.AppendLine("                }");
            sb.AppendLine("                float mean_mag = sqrtf(mean.x*mean.x + mean.y*mean.y + mean.z*mean.z);");
            sb.AppendLine("                if(citylbm_stg_min_streamwise_fraction > 0.0f) {");
            ...
            sb.AppendLine("            }");
            sb.AppendLine("        });");
            AppendSyntheticTurbulentInletDiagnosticsCode(sb, inletCondition, diagnosticsCsv, dx);
            sb.AppendLine("        lbm.flags.write_to_device();");
            sb.AppendLine("        lbm.u.write_to_device();");
            sb.AppendLine("#if defined(RECONSTRUCT_INLET_STRESS_DDF)");
            sb.AppendLine("        lbm.reconstruct_inlet_stress_boundaries();");
            sb.AppendLine("#elif defined(RECONSTRUCT_EQUILIBRIUM_BOUNDARY_DDF)");
            sb.AppendLine("        lbm.reconstruct_equilibrium_boundaries();");
            sb.AppendLine("#endif");
            sb.AppendLine("    };");
        ...
        private void AppendSyntheticTurbulentInletDiagnosticsCode(StringBuilder sb, string inletCondition, string diagnosticsCsv, double dx)
        {
            sb.AppendLine("        // Runtime inlet diagnostics: written by the generated FluidX3D case, not by post-hoc CityLBM scripts.");
            sb.AppendLine($"        const char* citylbm_inlet_diagnostics_csv = \"{diagnosticsCsv}\";");
            sb.AppendLine($"        const float citylbm_grid_dx_m = {dx.ToString("F8", CultureInfo.InvariantCulture)}f;");
            sb.AppendLine("        if(t_step == 0u) {");
            sb.AppendLine("            std::ofstream citylbm_diag_header(citylbm_inlet_diagnostics_csv, std::ios::out);");
            ...
            sb.AppendLine("        }");
            sb.AppendLine("        std::ofstream citylbm_diag(citylbm_inlet_diagnostics_csv, std::ios::app);");
            sb.AppendLine("        float citylbm_t1x = -dir_y;");
            sb.AppendLine("        float citylbm_t1y = dir_x;");
            sb.AppendLine("        float citylbm_t1z = 0.0f;");
            sb.AppendLine("        float citylbm_t1_norm = sqrtf(citylbm_t1x*citylbm_t1x + citylbm_t1y*citylbm_t1y + citylbm_t1z*citylbm_t1z);");
            sb.AppendLine("        if(citylbm_t1_norm < 1.0e-6f) { citylbm_t1x = 1.0f; citylbm_t1y = 0.0f; citylbm_t1z = 0.0f; citylbm_t1_norm = 1.0f; }");
            sb.AppendLine("        citylbm_t1x /= citylbm_t1_norm; citylbm_t1y /= citylbm_t1_norm; citylbm_t1z /= citylbm_t1_norm;");
            sb.AppendLine("        for(int citylbm_pi=0; citylbm_pi<profile_count; citylbm_pi++) {");
            sb.AppendLine("            int sample_z = (int)floorf((profile_z_m[citylbm_pi] - profile_origin_z_m) / citylbm_grid_dx_m);");
            sb.AppendLine("            if(sample_z < 1) sample_z = 1;");
            sb.AppendLine("            if(sample_z > (int)Nz - 1) sample_z = (int)Nz - 1;");
            sb.AppendLine("            double sum_u = 0.0, sum_v = 0.0, sum_w = 0.0;");
            sb.AppendLine("            double sum_u2 = 0.0, sum_v2 = 0.0, sum_w2 = 0.0;");
            sb.AppendLine("            double sum_x = 0.0, sum_y = 0.0, sum_z = 0.0;");
            sb.AppendLine("            double sum_x2 = 0.0, sum_y2 = 0.0, sum_z2 = 0.0;");
            sb.AppendLine("            double sum_xy = 0.0, sum_xz = 0.0, sum_yz = 0.0;");
            sb.AppendLine("            ulong sample_count = 0ull;");
            sb.AppendLine("            for(ulong n=0ull; n<lbm.get_N(); n++) {");
            sb.AppendLine("                uint x=0u, y=0u, z=0u;");
            sb.AppendLine("                lbm.coordinates(n, x, y, z);");
            sb.AppendLine($"                if(lbm.flags[n] == TYPE_E && {inletCondition} && z == (uint)sample_z) {{");
            sb.AppendLine("                    float ux = lbm.u.x[n] * citylbm_velocity_scale_lbm_to_mps;");
            sb.AppendLine("                    float uy = lbm.u.y[n] * citylbm_velocity_scale_lbm_to_mps;");
            sb.AppendLine("                    float uz = lbm.u.z[n] * citylbm_velocity_scale_lbm_to_mps;");
            sb.AppendLine("                    float stream = ux*dir_x + uy*dir_y + uz*dir_z;");
            sb.AppendLine("                    float cross1 = ux*citylbm_t1x + uy*citylbm_t1y + uz*citylbm_t1z;");
            sb.AppendLine("                    float cross2 = uz;");
            sb.AppendLine("                    sum_u += stream; sum_v += cross1; sum_w += cross2;");
            sb.AppendLine("                    sum_u2 += stream*stream; sum_v2 += cross1*cross1; sum_w2 += cross2*cross2;");
            sb.AppendLine("                    sum_x += ux; sum_y += uy; sum_z += uz;");
            sb.AppendLine("                    sum_x2 += ux*ux; sum_y2 += uy*uy; sum_z2 += uz*uz;");
            sb.AppendLine("                    sum_xy += ux*uy; sum_xz += ux*uz; sum_yz += uy*uz;");
            sb.AppendLine("                    sample_count++;");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine("            if(sample_count > 0ull) {");
            sb.AppendLine("                double inv = 1.0 / (double)sample_count;");
            sb.AppendLine("                double mean_u = sum_u * inv;");
            sb.AppendLine("                double mean_v = sum_v * inv;");
            sb.AppendLine("                double mean_w = sum_w * inv;");
            sb.AppendLine("                double rms_u = sqrt(fmax(0.0, sum_u2 * inv - mean_u * mean_u));");
            sb.AppendLine("                double rms_v = sqrt(fmax(0.0, sum_v2 * inv - mean_v * mean_v));");
            sb.AppendLine("                double rms_w = sqrt(fmax(0.0, sum_w2 * inv - mean_w * mean_w));");
            sb.AppendLine("                double measured_k = 0.5 * (rms_u*rms_u + rms_v*rms_v + rms_w*rms_w);");
            sb.AppendLine("                double mean_x = sum_x * inv;");
            sb.AppendLine("                double mean_y = sum_y * inv;");
            sb.AppendLine("                double mean_z = sum_z * inv;");
            sb.AppendLine("                double measured_r11 = fmax(0.0, sum_x2 * inv - mean_x * mean_x);");
            sb.AppendLine("                double measured_r22 = fmax(0.0, sum_y2 * inv - mean_y * mean_y);");
            sb.AppendLine("                double measured_r33 = fmax(0.0, sum_z2 * inv - mean_z * mean_z);");
            sb.AppendLine("                double measured_r12 = sum_xy * inv - mean_x * mean_y;");
            sb.AppendLine("                double measured_r13 = sum_xz * inv - mean_x * mean_z;");
            sb.AppendLine("                double measured_r23 = sum_yz * inv - mean_y * mean_z;");
            sb.AppendLine("                double rms_x = sqrt(measured_r11);");
            sb.AppendLine("                double rms_y = sqrt(measured_r22);");
            sb.AppendLine("                double rms_z = sqrt(measured_r33);");
            sb.AppendLine("                float sample_z_m = profile_origin_z_m + ((float)sample_z + 0.5f) * citylbm_grid_dx_m;");
            sb.AppendLine("                float target_u = profile_u_lbm[citylbm_pi] * citylbm_velocity_scale_lbm_to_mps;");
            sb.AppendLine("                float target_k = profile_k_m2s2[citylbm_pi];");
            sb.AppendLine("                float citylbm_variance_scale_lbm_to_m2s2 = citylbm_velocity_scale_lbm_to_mps * citylbm_velocity_scale_lbm_to_mps;");
            sb.AppendLine("                float target_r11 = profile_r11_lbm[citylbm_pi] * citylbm_variance_scale_lbm_to_m2s2;");
            sb.AppendLine("                float target_r22 = profile_r22_lbm[citylbm_pi] * citylbm_variance_scale_lbm_to_m2s2;");
            sb.AppendLine("                float target_r33 = profile_r33_lbm[citylbm_pi] * citylbm_variance_scale_lbm_to_m2s2;");
            sb.AppendLine("                float target_r12 = profile_r12_lbm[citylbm_pi] * citylbm_variance_scale_lbm_to_m2s2;");
            sb.AppendLine("                float target_r13 = profile_r13_lbm[citylbm_pi] * citylbm_variance_scale_lbm_to_m2s2;");
            sb.AppendLine("                float target_r23 = profile_r23_lbm[citylbm_pi] * citylbm_variance_scale_lbm_to_m2s2;");
            sb.AppendLine("                float3 target_sigma = citylbm_stg_target_sigma_xyz((uint)sample_z, windProfile((uint)sample_z));");
            sb.AppendLine("                float target_sx = target_sigma.x * citylbm_velocity_scale_lbm_to_mps;");
            sb.AppendLine("                float target_sy = target_sigma.y * citylbm_velocity_scale_lbm_to_mps;");
            sb.AppendLine("                float target_sz = target_sigma.z * citylbm_velocity_scale_lbm_to_mps;");
            sb.AppendLine("                float target_stream_rms = sqrtf((target_sx*dir_x)*(target_sx*dir_x) + (target_sy*dir_y)*(target_sy*dir_y) + (target_sz*dir_z)*(target_sz*dir_z));");
            sb.AppendLine("                float target_cross1_rms = sqrtf((target_sx*citylbm_t1x)*(target_sx*citylbm_t1x) + (target_sy*citylbm_t1y)*(target_sy*citylbm_t1y) + (target_sz*citylbm_t1z)*(target_sz*citylbm_t1z));");
            sb.AppendLine("                float target_cross2_rms = target_sz;");
            sb.AppendLine("                citylbm_diag << t_step << \",\" << citylbm_pi << \",\" << profile_z_m[citylbm_pi] << \",\" << sample_z");
            sb.AppendLine("                    << \",\" << target_u << \",\" << target_stream_rms << \",\" << target_cross1_rms << \",\" << target_cross2_rms << \",\" << target_k");
            sb.AppendLine("                    << \",\" << target_r11 << \",\" << target_r22 << \",\" << target_r33 << \",\" << target_r12 << \",\" << target_r13 << \",\" << target_r23");
            sb.AppendLine("                    << \",\" << mean_u << \",\" << mean_v << \",\" << mean_w << \",\" << rms_u << \",\" << rms_v << \",\" << rms_w << \",\" << measured_k");
            sb.AppendLine("                    << \",\" << mean_x << \",\" << mean_y << \",\" << mean_z << \",\" << rms_x << \",\" << rms_y << \",\" << rms_z");
            sb.AppendLine("                    << \",\" << measured_r11 << \",\" << measured_r22 << \",\" << measured_r33 << \",\" << measured_r12 << \",\" << measured_r13 << \",\" << measured_r23");
            sb.AppendLine("                    << \",\" << sample_count << \",\" << sample_z << \",\" << sample_z_m << \"\\n\";");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
        }
        ...
        private void SaveCaseMetadata(string caseDir, Scene scene, CartesianGrid grid, SimulationSettings settings)
        {
            try
            {
                ...
                int customProfileRmsRowCount = scene.CustomWindProfile == null ? 0 : scene.CustomWindProfile.Count(s => s.HasRms);
                int customProfileReynoldsStressRowCount = scene.CustomWindProfile == null ? 0 : scene.CustomWindProfile.Count(s => s.HasReynoldsStressTensor);
                bool customProfileKComplete = customProfileRowCount > 0 && customProfileKRowCount == customProfileRowCount;
                bool customProfileRmsComplete = customProfileRowCount > 0 && customProfileRmsRowCount == customProfileRowCount;
                bool customProfileReynoldsStressComplete = customProfileRowCount > 0 && customProfileReynoldsStressRowCount == customProfileRowCount;
                ...
                double minURmsMps = customProfileRmsComplete ? scene.CustomWindProfile.Min(s => s.URms) : 0.0;
                double maxURmsMps = customProfileRmsComplete ? scene.CustomWindProfile.Max(s => s.URms) : 0.0;
                double minVRmsMps = customProfileRmsComplete ? scene.CustomWindProfile.Min(s => s.VRms) : 0.0;
                double maxVRmsMps = customProfileRmsComplete ? scene.CustomWindProfile.Max(s => s.VRms) : 0.0;
                double minWRmsMps = customProfileRmsComplete ? scene.CustomWindProfile.Min(s => s.WRms) : 0.0;
                double maxWRmsMps = customProfileRmsComplete ? scene.CustomWindProfile.Max(s => s.WRms) : 0.0;
                double minR11M2s2 = customProfileReynoldsStressComplete ? scene.CustomWindProfile.Min(s => s.R11) : 0.0;
                double maxR11M2s2 = customProfileReynoldsStressComplete ? scene.CustomWindProfile.Max(s => s.R11) : 0.0;
                double minR22M2s2 = customProfileReynoldsStressComplete ? scene.CustomWindProfile.Min(s => s.R22) : 0.0;
                double maxR22M2s2 = customProfileReynoldsStressComplete ? scene.CustomWindProfile.Max(s => s.R22) : 0.0;
                double minR33M2s2 = customProfileReynoldsStressComplete ? scene.CustomWindProfile.Min(s => s.R33) : 0.0;
                double maxR33M2s2 = customProfileReynoldsStressComplete ? scene.CustomWindProfile.Max(s => s.R33) : 0.0;
                ...
                int expectedFrames = ComputeExpectedVtkFrameCount(settings);
                int expectedPaperAverageStepSpan = ComputeExpectedFinalWindowStepSpan(settings, PaperRecommendedAveragingFrames);
                int adaptivePaperAverageFrames = ComputeRequiredAverageFramesForStepSpan(
                    settings.SaveInterval,
                    PaperRecommendedAveragingFrames,
                    PaperRecommendedAverageStepSpan);
                int expectedAdaptivePaperAverageStepSpan = ComputeExpectedFinalWindowStepSpan(settings, adaptivePaperAverageFrames);
                int expectedPaperAverageStgRefreshes = ComputeExpectedFinalWindowStgRefreshCount(settings, adaptivePaperAverageFrames);
                ...
                string inletReynoldsStressTreatment = syntheticActive
                    ? (customProfileReynoldsStressComplete
                        ? "measured_or_precursor_full_tensor_from_csv_R11_R22_R33_R12_R13_R23"
                        : (customProfileRmsComplete
                        ? "measured_diagonal_rms_from_af_R11_u_rms2_R22_v_rms2_R33_w_rms2; offdiagonal_covariances_missing"
                        : "isotropic_from_k_only_R11_R22_R33_2k_over_3_R12_R13_R23_0; no measured Reynolds-stress tensor in AF table"))
                    : (customProfileReynoldsStressComplete ? "metadata_only_full_tensor_from_csv_not_injected" : (hasK ? "metadata_only_isotropic_k_assumption_not_injected" : "none"));
                bool inletReynoldsStressTensorAvailable = customProfileReynoldsStressComplete;
                string reynoldsStressAssumption = customProfileReynoldsStressComplete
                    ? "full Reynolds-stress tensor columns R11/R22/R33/R12/R13/R23 are read from CSV; per-layer mean correction plus covariance whitening/re-coloring preserves diagonal and off-diagonal Reynolds-stress targets before DDF reconstruction"
                    : (customProfileRmsComplete
                    ? "measured diagonal RMS is used for R11/R22/R33 and STG component targets; R12/R13/R23 are unavailable and emitted as zero diagnostic placeholders"
                    : (hasK ? "isotropic k only; no Reynolds stress tensor is available from AF table" : ""));
                string inletReynoldsStressTensorSource = syntheticActive
                    ? (customProfileReynoldsStressComplete
                        ? "full_reynolds_stress_tensor_from_CustomTable_CSV"
                        : (customProfileRmsComplete
                        ? "measured_diagonal_rms_from_AF_columns"
                        : "isotropic_k_assumption_from_AF_k_column"))
                    : (customProfileReynoldsStressComplete
                        ? "metadata_only_full_reynolds_stress_tensor_from_CustomTable_CSV"
                        : (customProfileRmsComplete
                        ? "metadata_only_measured_diagonal_rms_from_AF_columns"
                        : (hasK ? "metadata_only_isotropic_k_assumption_from_AF_k_column" : "none")));
                string r11Component = customProfileReynoldsStressComplete ? "R11_csv" : (customProfileRmsComplete ? "u_rms^2" : "2k/3");
                string r22Component = customProfileReynoldsStressComplete ? "R22_csv" : (customProfileRmsComplete ? "v_rms^2" : "2k/3");
                string r33Component = customProfileReynoldsStressComplete ? "R33_csv" : (customProfileRmsComplete ? "w_rms^2" : "2k/3");
                string r12Component = customProfileReynoldsStressComplete ? "R12_csv" : "0";
                string r13Component = customProfileReynoldsStressComplete ? "R13_csv" : "0";
                string r23Component = customProfileReynoldsStressComplete ? "R23_csv" : "0";
                string offDiagonalTreatment = customProfileReynoldsStressComplete
                    ? "R12/R13/R23 read from full tensor CSV and targeted by per_z_cell covariance whitening/re-coloring; paper-grade use still requires native R_ij preservation evidence"
                    : (customProfileRmsComplete
                    ? "R12=R13=R23 unavailable in AF RMS table; emitted as 0 diagnostic placeholders"
                    : (syntheticActive ? "R12=R13=R23=0 isotropic assumption" : "none"));
                bool supportsInletStressDdf = SupportsFluidX3DInletStressDdfReconstruction();
                bool supportsEquilibriumDdf = SupportsFluidX3DEquilibriumDdfReconstruction();
                bool inletDistributionFunctionReconstruction = syntheticActive && (supportsInletStressDdf || supportsEquilibriumDdf);
                string runtimeInletDiagnosticsCsv = syntheticActive
                    ? SanitizeName(scene.Name ?? "case").ToLowerInvariant() + "_inlet_turbulence_stats.csv"
                    : "";
                string inletDdfReconstructionRoute = supportsInletStressDdf
                    ? "fluidx3d_RECONSTRUCT_INLET_STRESS_DDF"
                    : (supportsEquilibriumDdf ? "fluidx3d_RECONSTRUCT_EQUILIBRIUM_BOUNDARY_DDF" : "none");
                bool boundaryNonReflectingOutletImplemented = false;
                bool boundarySideTopWindTunnelEquivalentImplemented = false;
                bool boundaryRoughWallFunctionImplemented = true;
                bool boundaryPrecursorOrRecyclingImplemented = false;
                bool boundaryBlockageFetchEvidenceArchived = false;
                var missingPaperGradeInletEvidenceList = new List<string>();
                if (!syntheticActive)
                    missingPaperGradeInletEvidenceList.Add("active_correlated_turbulent_inlet");
                if (!customProfileReynoldsStressComplete)
                    missingPaperGradeInletEvidenceList.Add("measured_or_precursor_reynolds_stress_tensor");
                if (!inletDistributionFunctionReconstruction)
                    missingPaperGradeInletEvidenceList.Add("inlet_distribution_function_reconstruction");
                missingPaperGradeInletEvidenceList.Add("empty_tunnel_U_k_correlation_preservation_gate");
                string[] missingPaperGradeInletEvidence = missingPaperGradeInletEvidenceList.ToArray();
                var missingPaperGradeBoundaryEvidenceList = new List<string>
                {
                ...
                if (!boundaryRoughWallFunctionImplemented)
                    missingPaperGradeBoundaryEvidenceList.Add("rough_wall_or_wall_function_action");
                string[] missingPaperGradeBoundaryEvidence = missingPaperGradeBoundaryEvidenceList.ToArray();
                var metadata = new
                {
                    SchemaVersion = 2,
                    ...
                    VelocityOutputUnits = "FluidX3D write_device_to_vtk exports lattice velocity; VTK validation readers should apply VelocityScaleLbmToMps before comparing with SI AF/RS data.",
                    VtkReaderShouldApplyVelocityScale = true,
                    ...
                    VtkSaveStartStep = settings.VtkSaveStartStep,
                    ExpectedVtkFrameCount = expectedFrames,
                    ExpectedVtkSourceTimeSteps = ComputeExpectedVtkSourceTimeSteps(settings).ToArray(),
                    VtkOutput = new
                    {
                        SaveIntervalSteps = settings.SaveInterval,
                        SaveStartStep = settings.VtkSaveStartStep,
                        ExpectedSourceTimeSteps = ComputeExpectedVtkSourceTimeSteps(settings).ToArray()
                    },
                    ...
                    PaperRecommendedAdaptiveAveragingFrames = adaptivePaperAverageFrames,
                    ExpectedAdaptivePaperAverageStepSpan = expectedAdaptivePaperAverageStepSpan,
                    TimeAveragingAdaptiveWindowPolicy = "average at least PaperRecommendedAveragingFrames and automatically increase the final-window frame count when SaveInterval is too small to span PaperRecommendedAverageStepSpan",
                    ...
                    TimeAveragingPaperGate = expectedFrames >= adaptivePaperAverageFrames && expectedAdaptivePaperAverageStepSpan >= PaperRecommendedAverageStepSpan
                        ? "pass_adaptive_paper_recommended_frame_count_and_step_span"
                        : "diagnostic_only_extend_time_steps_or_reduce_save_interval",
                    TimeAveragingRunGateRequiredForModes = "Mode 1/2/3 require ExpectedVtkFrameCount >= MinimumRecommendedAveragingFrames for a smoke/diagnostic workflow; paper-grade validation also requires PaperRecommendedAdaptiveAveragingFrames and PaperRecommendedAverageStepSpan.",
                    ...
                    CustomProfileHasMeasuredDiagonalRms = customProfileRmsComplete,
                    CustomProfileMeasuredDiagonalRmsRows = customProfileRmsRowCount,
                    CustomProfileMeasuredDiagonalRmsComplete = customProfileRmsComplete,
                    CustomProfileHasFullReynoldsStressTensor = customProfileReynoldsStressComplete,
                    CustomProfileReynoldsStressTensorRows = customProfileReynoldsStressRowCount,
                    CustomProfileReynoldsStressTensorComplete = customProfileReynoldsStressComplete,
                    KColumnStatus = hasK
                    ...
                    MeasuredDiagonalRmsColumnStatus = customProfileRmsComplete
                        ? "read_from_csv_and_used_for_STG_component_targets"
                        : "not_available",
                    URmsMinMps = minURmsMps,
                    URmsMaxMps = maxURmsMps,
                    VRmsMinMps = minVRmsMps,
                    VRmsMaxMps = maxVRmsMps,
                    WRmsMinMps = minWRmsMps,
                    WRmsMaxMps = maxWRmsMps,
                    R11MinM2s2 = minR11M2s2,
                    R11MaxM2s2 = maxR11M2s2,
                    R22MinM2s2 = minR22M2s2,
                    R22MaxM2s2 = maxR22M2s2,
                    R33MinM2s2 = minR33M2s2,
                    R33MaxM2s2 = maxR33M2s2,
                    TurbulentInletLevel = syntheticActive
                        ? (customProfileReynoldsStressComplete
                            ? "Level 3.0 STG-lite spectral perturbation from full Reynolds-stress tensor plus k metadata"
                            : (customProfileRmsComplete
                            ? "Level 2.6 STG-lite spectral perturbation from measured diagonal RMS plus k metadata"
                            : "Level 2.5 STG-lite spectral perturbation from isotropic k"))
                        : (customProfileReynoldsStressComplete ? "Level 3 metadata/diagnostic chain" : (hasK ? "Level 2 metadata/diagnostic chain" : "none")),
                    ...
                    RuntimeInletDiagnosticsCsv = runtimeInletDiagnosticsCsv,
                    RuntimeInletDiagnosticsWriter = syntheticActive
                        ? "generated_setup_cpp_applyInlet_writes_profile_layer_U_RMS_k_csv"
                        : "not_applicable",
                    RuntimeInletDiagnosticsColumns = syntheticActive
                        ? "step,profile_index,z_m,z_cell,target_U_mps,target_u_rms_mps,target_v_rms_mps,target_w_rms_mps,target_k_m2s2,target_r11_m2s2,target_r22_m2s2,target_r33_m2s2,target_r12_m2s2,target_r13_m2s2,target_r23_m2s2,mean_U_mps,mean_V_mps,mean_W_mps,u_rms_mps,v_rms_mps,w_rms_mps,k_m2s2,mean_x_mps,mean_y_mps,mean_z_mps,x_rms_mps,y_rms_mps,z_rms_mps,measured_r11_m2s2,measured_r22_m2s2,measured_r33_m2s2,measured_r12_m2s2,measured_r13_m2s2,measured_r23_m2s2,samples_y,effective_sample_z_cell,effective_sample_z_m"
                        : "",
                    SyntheticTurbulentInletMethod = syntheticActive
                        ? (customProfileReynoldsStressComplete
                            ? "STG-lite plus synthetic_eddy diagnostic inlet: deterministic divergence-reduced spectral modes transformed by full Reynolds-stress tensor Cholesky targets, persistent compact synthetic eddy population, Taylor frozen-turbulence advection and deterministic temporal filter state; TYPE_E DDF reconstruction is used when supported by the selected FluidX3D source"
                            : "STG-lite plus synthetic_eddy diagnostic inlet: deterministic divergence-reduced spectral modes, persistent compact synthetic eddies from isotropic k, Taylor frozen-turbulence advection and deterministic temporal filter state; TYPE_E DDF reconstruction is used when supported by the selected FluidX3D source")
                        : "none",
                    SyntheticTurbulentInletTemporalTreatment = syntheticActive
                        ? "continuous-step Taylor frozen-turbulence phase advection by local mean LBM velocity along the wind vector with temporal_step_scale_0.25 plus light deterministic_temporal_phase_blend_rho_0.65 and persistent synthetic_eddy centers advected/wrapped between updates"
                        : "none",
                    ...
                    SyntheticTurbulentInletEnergyNormalization = syntheticActive
                        ? (customProfileReynoldsStressComplete
                            ? "component target uses full Reynolds-stress tensor R11/R22/R33/R12/R13/R23 by height; deterministic projected-mode normalization plus per_z_cell inlet-face mean correction and covariance whitening/re-coloring preserve diagonal RMS, k and off-diagonal covariance targets after finite-mode sampling"
                            : (customProfileRmsComplete
                            ? "component RMS target uses measured u_rms/v_rms/w_rms by height; deterministic projected-mode normalization plus per_z_cell inlet-face RMS rescaling preserve those diagonal RMS targets after finite-mode sampling and mean correction"
                    ...
                    SyntheticTurbulentInletDistributionTreatment = syntheticActive
                        ? (inletDistributionFunctionReconstruction
                            ? $"{inletDdfReconstructionRoute}; refreshed on TYPE_E inlet nodes in batch and graphics modes after macroscopic velocity update"
                            : "velocity_field_only_no_distribution_function_reconstruction; refreshed on TYPE_E inlet nodes in batch and graphics modes")
                        : "none",
                    FluidX3DInletDdfReconstructionSupport = syntheticActive
                        ? inletDdfReconstructionRoute
                        : "not_applicable",
                    ...
                    SyntheticTurbulentInletMeanPreservingTreatment = syntheticActive
                        ...
                        : "none",
                    SyntheticTurbulentInletFullTensorCovariancePreservingCorrection = syntheticActive && customProfileReynoldsStressComplete,
                    ...
                    SyntheticTurbulentInletLayerwiseRmsPreservingTreatment = syntheticActive
                        ? (customProfileReynoldsStressComplete
                            ? "after mean correction, each inlet z_cell layer is whitened against the sampled covariance and re-colored with the target Reynolds-stress Cholesky factor so R11/R22/R33 and R12/R13/R23 match the full-tensor target before optional streamwise clipping"
                            : (customProfileRmsComplete
                            ? "after mean correction, each inlet z_cell layer is rescaled per component so the actual inlet-face RMS matches measured u_rms/v_rms/w_rms before optional streamwise clipping"
                    ...
                    ReynoldsStressAssumption = reynoldsStressAssumption,
                    InletReynoldsStressTensorSource = inletReynoldsStressTensorSource,
                    ...
                    InletReynoldsStressComponents = syntheticActive
                        ...
                        : null,
                    InletReynoldsStressOffDiagonalTreatment = offDiagonalTreatment,
                    WallRoughnessTreatment = "ground/buildings are voxelized TYPE_S no-slip plus diagnostic near-ground equivalent rough-wall drag written into FluidX3D FORCE_FIELD from RoughnessLength; not a complete wind-tunnel wall-function validation by itself",
                    ...
                    CoordinateProtocol = new
                    {
                        Axes = new
                        {
                            x = "streamwise; positive downstream in CityLBM validation post-processing coordinates",
                            y = "lateral/spanwise",
                            z = "vertical; floor at z=0 m"
                        },
                        VelocityComponents = new
                        {
                            U = "streamwise velocity compared with FluidX3D u.x; world-space workflows must archive the wind-vector projection mapping"
                        },
                        Normalization = new
                        {
                            Uref_mps = scene.WindSpeed,
                            Zref_m = scene.ReferenceHeight,
                            OutputRatios = new[] { "Ux_over_Uref", "Umag_over_Uref" },
                            Basis = "AIJ validation compares the selected streamwise component divided by the archived reference wind speed"
                        },
                        ProbeProjection = new
                        {
                            Formula = "(world_coordinate_m - DomainOriginM) / dx",
                    ...
                    BoundaryConditionMethodClass = "citylbm_type_e_profile_maintenance_buffer_diagnostic",
                    ...
                    BoundaryVelocityInitializationMethod = "fixed_mean_velocity_equilibrium_for_all_TYPE_E_faces_plus_profile_maintenance_FORCE_FIELD_buffer",
                    BoundaryProfileMaintenanceBufferApplied = true,
                    BoundaryProfileMaintenanceBufferTreatment = "outlet_lateral_top_interior_cells_receive_small_profile_aligned_FORCE_FIELD_buffer_to_reduce_open_box_profile_collapse; diagnostic_not_non_reflecting",
                    BoundaryProfileMaintenanceBufferLayerCells = 8.0,
                    BoundaryProfileMaintenanceBufferGain = 0.0015,
                    BoundaryProfileMaintenanceBufferMaxForceLbm = 0.0003,
                    BoundaryOutletTreatment = "TYPE_E_fixed_mean_velocity_equilibrium_plus_profile_maintenance_FORCE_FIELD_buffer_not_validated_pressure_or_non_reflecting_outlet",
                    BoundarySideTopTreatment = "TYPE_E_fixed_mean_velocity_equilibrium_plus_profile_maintenance_FORCE_FIELD_buffer_not_periodic_or_wind_tunnel_equivalent",
                    BoundaryRoughnessBoundaryTreatment = "TYPE_S_no_slip_plus_near_ground_equivalent_rough_wall_drag_FORCE_FIELD_from_RoughnessLength",
                    BoundaryDevelopmentTreatment = "none_no_precursor_or_recycling_development_field",
                    BoundaryFixedMeanVelocityOutletRisk = "outlet_lateral_top_mean_velocity_equilibrium_and_profile_maintenance_buffer_can_still_bias_pedestrian_level_velocities_requires_boundary_runtime_audit",
                    BoundaryVelocityInitializationPaperGradeStatus = "diagnostic_damping_mitigation_and_profile_buffer_not_wind_tunnel_equivalent_boundary",
                    ...
                    CustomProfile = scene.CustomWindProfile == null ? null : scene.CustomWindProfile.Select(s => new
                    {
                        ...
                        HasMeasuredDiagonalRms = s.HasRms,
                        URmsMps = s.HasRms ? s.URms : 0.0,
                        VRmsMps = s.HasRms ? s.VRms : 0.0,
                        WRmsMps = s.HasRms ? s.WRms : 0.0,
                        URmsLbm = s.HasRms ? s.URms * uScale : 0.0,
                        VRmsLbm = s.HasRms ? s.VRms * uScale : 0.0,
                        WRmsLbm = s.HasRms ? s.WRms * uScale : 0.0,
                        HasFullReynoldsStressTensor = s.HasReynoldsStressTensor,
                        R11M2s2 = s.HasReynoldsStressTensor ? s.R11 : 0.0,
                        R22M2s2 = s.HasReynoldsStressTensor ? s.R22 : 0.0,
                        R33M2s2 = s.HasReynoldsStressTensor ? s.R33 : 0.0,
                        R12M2s2 = s.HasReynoldsStressTensor ? s.R12 : 0.0,
                        R13M2s2 = s.HasReynoldsStressTensor ? s.R13 : 0.0,
                        R23M2s2 = s.HasReynoldsStressTensor ? s.R23 : 0.0,
                        R11Lbm = s.HasReynoldsStressTensor ? s.R11 * uScale * uScale : 0.0,
                        R22Lbm = s.HasReynoldsStressTensor ? s.R22 * uScale * uScale : 0.0,
                        R33Lbm = s.HasReynoldsStressTensor ? s.R33 * uScale * uScale : 0.0,
                        R12Lbm = s.HasReynoldsStressTensor ? s.R12 * uScale * uScale : 0.0,
                        R13Lbm = s.HasReynoldsStressTensor ? s.R13 * uScale * uScale : 0.0,
                        R23Lbm = s.HasReynoldsStressTensor ? s.R23 * uScale * uScale : 0.0
                    }).ToList()
                };
        ...
        internal static string InferAijCaseLabel(string sceneName)
        {
            string text = (sceneName ?? "").ToLowerInvariant();
            if (text.Contains("casea") || text.Contains("case_a") || text.Contains("case-a") || text.Contains("aij_a"))
                return "CaseA";
            if (text.Contains("casee") || text.Contains("case_e") || text.Contains("case-e") || text.Contains("aij_e"))
                return "CaseE";
            return "";
        }
        ...
        private BoundaryProtocolAudit BuildBoundaryProtocolAudit(Scene scene, CartesianGrid grid)
        {
            ...
            return new BoundaryProtocolAudit
            {
                ...
                BoundaryTypes = new BoundaryTypesRecord
                {
                    ...
                    Outlet = "TYPE_E pressure/free-outflow approximation; velocity initialized from mean profile plus diagnostic profile-maintenance FORCE_FIELD buffer in interior cells",
                    Lateral = "TYPE_E slip/free approximation; velocity initialized from mean profile plus diagnostic profile-maintenance FORCE_FIELD buffer in interior cells",
                    Top = "TYPE_E; velocity initialized from mean profile plus diagnostic profile-maintenance FORCE_FIELD buffer in interior cells",
                    Ground = "TYPE_S no-slip plus diagnostic near-ground equivalent rough-wall FORCE_FIELD from RoughnessLength",
        ...
        private static int ComputeExpectedFinalWindowStepSpan(SimulationSettings settings, int averageFrameCount)
        {
            ...
            var savedSteps = ComputeExpectedVtkSourceTimeSteps(settings);
        ...
        private static List<int> ComputeExpectedVtkSourceTimeSteps(SimulationSettings settings)
        {
            var savedSteps = new List<int>();
            if (settings == null || settings.SaveInterval <= 0 || settings.TimeSteps <= 0)
                return savedSteps;
        ...
        private static int ComputeExpectedVtkFrameCount(SimulationSettings settings)
        {
            return ComputeExpectedVtkSourceTimeSteps(settings).Count;
        }
        ...
        private static int ComputeRequiredAverageFramesForStepSpan(int saveInterval, int minFrames, int minStepSpan)
        {
        ...
        private IEnumerable<string> BuildProtocolRisks(Scene scene, SimulationSettings settings)
        {
                ...
                scene.CustomWindProfile.Any(s => s.HasK))
            {
                if (syntheticActive)
                {
                    ...
                    yield return SupportsFluidX3DInletStressDdfReconstruction() || SupportsFluidX3DEquilibriumDdfReconstruction()
                        ? "STG-lite inlet refreshes macroscopic lbm.u values on TYPE_E inlet nodes in both batch and graphics modes, then calls FluidX3D TYPE_E DDF reconstruction; k preservation must still be proven by an empty-tunnel native baseline before paper-grade validation."
                        : "STG-lite inlet refreshes macroscopic lbm.u values on TYPE_E inlet nodes in both batch and graphics modes; the selected FluidX3D source lacks DDF reconstruction hooks, so k preservation cannot be treated as paper-grade.";
                    ...
                    int adaptiveAverageFrames = ComputeRequiredAverageFramesForStepSpan(
                        settings.SaveInterval,
                        PaperRecommendedAveragingFrames,
                        PaperRecommendedAverageStepSpan);
                    int expectedStgRefreshes = ComputeExpectedFinalWindowStgRefreshCount(settings, adaptiveAverageFrames);
            ...
            yield return "Boundary conditions use TYPE_E inlet/outlet/lateral/top approximations plus a profile-maintenance FORCE_FIELD buffer and must be checked against the AIJ wind-tunnel protocol.";
            yield return "CityLBM initializes all TYPE_E boundary velocities from the mean wind profile before device upload and applies a small outlet/lateral/top interior FORCE_FIELD buffer to reduce zero-speed/open-box profile damping.";
            ...
            yield return "Ground/buildings are TYPE_S no-slip with a diagnostic near-ground equivalent rough-wall FORCE_FIELD from RoughnessLength; AF profile roughness and this force-field correction still require AIJ protocol/runtime validation.";
            ...
            int adaptivePaperAverageFrames = ComputeRequiredAverageFramesForStepSpan(
                ...
                PaperRecommendedAverageStepSpan);
            int expectedAdaptivePaperAverageStepSpan = ComputeExpectedFinalWindowStepSpan(settings, adaptivePaperAverageFrames);
            if (expectedFrames < adaptivePaperAverageFrames || expectedAdaptivePaperAverageStepSpan < PaperRecommendedAverageStepSpan)
            {
                yield return $"Only {expectedFrames} VTK frames are expected after VtkSaveStartStep={settings.VtkSaveStartStep}; a fixed {PaperRecommendedAveragingFrames}-frame span is {expectedPaperAverageStepSpan}, and the adaptive paper window needs {adaptivePaperAverageFrames} late frames spanning about {PaperRecommendedAverageStepSpan} solver steps after stationarity is checked.";
            }
            ...
            try
            {
                ...
                var manifest = new
                {
                    ...
                    SharedRunConditions = new
                    {
                        ...
                        SaveInterval = settings.SaveInterval,
                        VtkSaveStartStep = settings.VtkSaveStartStep,
                        ExpectedVtkFrameCount = ComputeExpectedVtkFrameCount(settings),
                        ExpectedVtkSourceTimeSteps = ComputeExpectedVtkSourceTimeSteps(settings).ToArray(),
                        GeometryPhysicalUnitAssumption = "Rhino_model_geometry_is_already_real_scale_meters_before_case_generation",
                        ...
                        PaperRecommendedAdaptiveAveragingFrames = ComputeRequiredAverageFramesForStepSpan(settings.SaveInterval, PaperRecommendedAveragingFrames, PaperRecommendedAverageStepSpan),
                        ExpectedAdaptivePaperAverageStepSpan = ComputeExpectedFinalWindowStepSpan(settings, ComputeRequiredAverageFramesForStepSpan(settings.SaveInterval, PaperRecommendedAveragingFrames, PaperRecommendedAverageStepSpan)),
                        WindProfile = scene.WindProfile.ToString(),
                        WindProfileCsvPath = scene.WindProfileCsvPath ?? "",
                        ...
                        SyntheticTurbulenceStreamwiseClippingTreatment = settings.SyntheticTurbulenceMinStreamwiseFraction > 0.0
                            ...
                            : "disabled_no_streamwise_clipping_of_k_perturbations",
                        SyntheticTurbulentInletTemporalTreatment = syntheticActive
                            ? "continuous-step Taylor frozen-turbulence phase advection by local mean LBM velocity along the wind vector with temporal_step_scale_0.25 plus light deterministic_temporal_phase_blend_rho_0.65 and persistent synthetic_eddy centers advected/wrapped between cached plane updates"
                            : "none",
                        SyntheticTurbulentInletDistributionTreatment = syntheticActive
                            ? (inletDistributionFunctionReconstruction
                                ? $"{inletDdfReconstructionRoute}; refreshed on TYPE_E inlet nodes after macroscopic velocity update"
                        ...
                        PaperGradeTurbulentInletPrerequisiteGate = "fail",
                        PaperGradeTurbulentInletMissingEvidence = missingPaperGradeInletEvidence,
                        WallRoughnessTreatment = "ground/buildings TYPE_S no-slip plus diagnostic near-ground equivalent rough-wall drag written into FluidX3D FORCE_FIELD from RoughnessLength; not a complete wind-tunnel wall-function validation by itself",
                        BoundaryConditionSummary = GetBoundaryConditionSummary(scene.WindDirection, scene.WindProfile),
                        BoundaryConditionMethodClass = "citylbm_type_e_profile_maintenance_buffer_diagnostic",
                        BoundaryConditionPaperGradeStatus = "diagnostic_only_until_boundary_source_and_aij_protocol_evidence_pass",
                        BoundaryTypeEVelocityInitializationApplied = true,
        ...
        private IEnumerable<ValidationProtocolAuditItem> BuildValidationProtocolAuditItems(Scene scene, CartesianGrid grid, SimulationSettings settings)
        {
            ...
            bool inletDistributionFunctionReconstruction = syntheticActive &&
                (SupportsFluidX3DInletStressDdfReconstruction() || SupportsFluidX3DEquilibriumDdfReconstruction());
            string inletDdfReconstructionRoute = SupportsFluidX3DInletStressDdfReconstruction()
                ? "RECONSTRUCT_INLET_STRESS_DDF"
                : (SupportsFluidX3DEquilibriumDdfReconstruction() ? "RECONSTRUCT_EQUILIBRIUM_BOUNDARY_DDF" : "none");
            ...
            yield return new ValidationProtocolAuditItem
            {
                ...
                Evidence = syntheticActive
                    ? (hasFullReynoldsStressTensor
                        ? $"AF k column and full Reynolds-stress tensor columns are present; setup.cpp will emit syntheticTurbulentInlet/applySyntheticTurbulentInlet with {settings.SyntheticTurbulenceModeCount} spectral modes, Cholesky covariance targets, continuous-step Taylor frozen-turbulence advection, deterministic temporal phase blending, TYPE_E inlet refreshes in batch/graphics modes and DDF route={inletDdfReconstructionRoute}."
                        : (hasMeasuredDiagonalRms
                        ? $"AF k column and measured u_rms/v_rms/w_rms columns are present; setup.cpp will emit syntheticTurbulentInlet/applySyntheticTurbulentInlet with {settings.SyntheticTurbulenceModeCount} spectral modes, per-component RMS normalization from measured diagonal RMS, continuous-step Taylor frozen-turbulence advection, deterministic temporal phase blending, TYPE_E inlet refreshes in batch/graphics modes and DDF route={inletDdfReconstructionRoute}."
                        : $"AF k column is present and STG-lite inlet is requested; setup.cpp will emit syntheticTurbulentInlet/applySyntheticTurbulentInlet with {settings.SyntheticTurbulenceModeCount} spectral modes, per-component RMS normalization from k, continuous-step Taylor frozen-turbulence advection, deterministic temporal phase blending, TYPE_E inlet refreshes in batch/graphics modes and DDF route={inletDdfReconstructionRoute}."))
                ...
                Risk = syntheticActive
                    ? $"STG-lite is not a full digital-filter/precursor/Reynolds-stress inlet, assumes isotropic turbulence and continuous-step frozen-turbulence advection with deterministic temporal phase blending, uses {settings.SyntheticTurbulenceModeCount} spectral modes (strict baseline minimum {StrictBaselineSyntheticTurbulenceModes}; mode gate={(syntheticModeCountStrict ? "pass" : "diagnostic_only_low_spectral_mode_count")}) and correlation length {settings.SyntheticTurbulenceCorrelationCells:F3} cells ({settings.SyntheticTurbulenceCorrelationCells * grid.Dx:F3} m) with source '{lengthScaleSource}', streamwise clipping fraction {settings.SyntheticTurbulenceMinStreamwiseFraction:F3}, and DDF reconstruction gate={(inletDistributionFunctionReconstruction ? "enabled" : "missing")}."
            ...
            yield return new ValidationProtocolAuditItem
            {
                ...
                Risk = syntheticActive
                    ? (lengthScaleSupported
                        ? (inletDistributionFunctionReconstruction
                            ? (hasFullReynoldsStressTensor
                                ? "Length-scale source, full tensor source and DDF reconstruction route are traceable, but STG-lite still needs native U/k/correlation preservation evidence."
                                : "Length-scale source and DDF reconstruction route are traceable, but STG-lite still lacks measured/precursor Reynolds-stress tensors and native U/k/correlation preservation evidence.")
                            : (hasFullReynoldsStressTensor
                                ? "Length-scale source and full tensor source are traceable, but STG-lite still lacks distribution-function reconstruction and native U/k/correlation preservation evidence."
                                : "Length-scale source is traceable, but STG-lite still lacks distribution-function reconstruction, Reynolds-stress tensors and native U/k/correlation preservation evidence."))
            ...
            yield return new ValidationProtocolAuditItem
            {
                ...
                Status = syntheticActive ? (hasFullReynoldsStressTensor ? "partial" : "risk") : (hasK ? "risk" : "fail"),
                Evidence = syntheticActive
                    ? (hasFullReynoldsStressTensor
                        ? "STG-lite reads full Reynolds-stress tensor columns R11/R22/R33/R12/R13/R23 from the CustomTable CSV and applies per-layer covariance whitening/re-coloring before DDF reconstruction; runtime gates must still prove U/k/Rij/correlation preservation."
                        : (hasMeasuredDiagonalRms
                        ? "STG-lite uses measured diagonal RMS from AF columns for R11=u_rms^2, R22=v_rms^2 and R33=w_rms^2; R12/R13/R23 are still unavailable and emitted as zero diagnostic placeholders."
                ...
                Risk = hasFullReynoldsStressTensor
                    ? "A full tensor input removes the zero-shear placeholder, but STG-lite still must be proven against empty-tunnel and building baselines before paper-grade accuracy claims."
                    : (hasMeasuredDiagonalRms
                        ? "Measured diagonal RMS reduces the isotropic-k approximation, but missing shear stresses/off-diagonal covariances can still bias separation, recirculation and pedestrian-level speed ratios."
                        : "A k-only isotropic inlet can match turbulence kinetic energy while missing anisotropy and shear stresses that control separation, recirculation and pedestrian-level speed ratios."),
                RequiredNextAction = hasFullReynoldsStressTensor
                    ? "Archive the tensor CSV/source, run empty-tunnel U/k/Rij preservation gates, then run building Case A/E probe metric gates before paper claims."
                    : (hasMeasuredDiagonalRms
                        ? "For paper-grade claims, archive measured/precursor off-diagonal Reynolds stresses or prove with empty-tunnel and building baselines that the measured-diagonal-only approximation preserves the target U/k/RMS/correlation envelope."
                        : "For paper-grade claims, archive a measured/precursor Reynolds-stress tensor, or prove with empty-tunnel and building baselines that the isotropic-k approximation preserves the target U/k/correlation envelope.")
            };
            ...
            yield return new ValidationProtocolAuditItem
            {
                ...
                Status = syntheticActive
                    ...
                    : "partial",
                Evidence = syntheticActive
                    ? $"STG Update={settings.SyntheticTurbulenceUpdateInterval} steps; adaptive final {adaptivePaperAverageFrames}-frame average spans {expectedAdaptivePaperAverageStepSpan} solver steps and is expected to sample {expectedPaperAverageStgRefreshes} inlet refreshes; recommended minimum={PaperRecommendedStgRefreshes}."
            ...
            yield return new ValidationProtocolAuditItem
            {
                ...
                Status = syntheticActive && inletDistributionFunctionReconstruction ? "partial" : "fail",
                Evidence = syntheticActive
                    ? (inletDistributionFunctionReconstruction
                        ? $"Generated setup refreshes inlet lbm.u and calls FluidX3D {inletDdfReconstructionRoute} after each TYPE_E inlet update."
                        : "Generated setup refreshes inlet lbm.u but the selected FluidX3D source does not expose usable DDF reconstruction hooks for the imposed turbulent fluctuation.")
                    : (hasK ? "AF k is available, but no turbulent fluctuation is injected into the inlet." : "No k-driven inlet path is active."),
                Risk = inletDistributionFunctionReconstruction
                    ? "DDF reconstruction removes the velocity-only inlet defect, but it is still a diagnostic STG-lite route until empty-tunnel U/k/correlation preservation and Reynolds-stress evidence pass."
                    : "A velocity-only turbulent inlet can dissipate or distort k near the boundary and may cause large Case A/Case E speed-ratio bias even when the GH workflow runs.",
                RequiredNextAction = "For SCI-grade validation, pass an empty-tunnel U/k preservation gate and then the building Case A/Case E probe metric gates; replace STG-lite with a validated DFM/SEM/precursor/recycling inlet if these gates fail."
            };
            ...
            yield return new ValidationProtocolAuditItem
            {
                ...
                Evidence = GetBoundaryConditionSummary(scene.WindDirection, scene.WindProfile) +
                    $" Generated setup also applies BoundaryProfileMaintenanceBufferApplied=true as a small outlet/lateral/top FORCE_FIELD profile-maintenance buffer. BoundaryProtocolAudit.Gate={boundaryAudit.Gate}; BoundaryProtocolEvidenceGate={boundaryAudit.ProtocolEvidenceGate}.",
                Risk = boundaryClearanceOk
                    ? "Clearance satisfies diagnostic defaults, but TYPE_E inlet/outlet/lateral/top plus the profile-maintenance buffer remains a simplified protocol without archived AIJ-equivalent boundary/fetch/roughness evidence."
                    : "Domain clearance or simplified TYPE_E/profile-maintenance boundary treatment can contaminate the validation field and contribute to systematic bias.",
            ...
            yield return new ValidationProtocolAuditItem
            {
                ...
                Status = "partial",
                Evidence = $"RoughnessLength={scene.RoughnessLength:F6} m; ground/buildings are generated as TYPE_S no-slip and setup.cpp writes a near-ground equivalent rough-wall drag into FluidX3D FORCE_FIELD.",
                Risk = "The force-field roughness correction is a real solver input, but it is still a diagnostic equivalent model; without AIJ floor/fetch evidence or empty-tunnel U/k preservation, pedestrian-height speed ratios and k can remain biased.",
                RequiredNextAction = "Before paper-grade claims, document the wind-tunnel floor treatment and validate the generated rough-wall force against an empty-tunnel U/k gate or replace it with a documented precursor/recycling rough-wall strategy."
            };
            ...
            {
                Key = "time_averaging",
                Status = expectedFrames >= adaptivePaperAverageFrames && expectedAdaptivePaperAverageStepSpan >= PaperRecommendedAverageStepSpan ? "partial" : "fail",
                Evidence = $"TimeSteps={settings.TimeSteps}, SaveInterval={settings.SaveInterval}, VtkSaveStartStep={settings.VtkSaveStartStep}, ExpectedVtkFrameCount={expectedFrames}, PaperRecommendedAveragingFrames={PaperRecommendedAveragingFrames}, PaperRecommendedAdaptiveAveragingFrames={adaptivePaperAverageFrames}, PaperRecommendedAverageStepSpan={PaperRecommendedAverageStepSpan}, ExpectedPaperAverageStepSpan={expectedPaperAverageStepSpan}, ExpectedAdaptivePaperAverageStepSpan={expectedAdaptivePaperAverageStepSpan}.",
                Risk = expectedFrames >= adaptivePaperAverageFrames && expectedAdaptivePaperAverageStepSpan >= PaperRecommendedAverageStepSpan
                ...
                RequiredNextAction = $"Use Read VTK Average Last N={adaptivePaperAverageFrames} or higher, archive the actual SourceTimeSteps used for metrics, and verify stationarity before interpreting validation error."
            };
    ...
    {
        public const int AijValidationBaselineTimeSteps = 40000;
        public const int AijValidationBaselineSaveInterval = 1000;
        public const int AijValidationBaselineSyntheticTurbulenceModeCount = 128;
        public const int AijValidationBaselineSyntheticTurbulenceUpdateInterval = 25;
        public const double AijValidationBaselineSyntheticTurbulenceIntensityScale = 1.0;
        public const double AijValidationBaselineSyntheticTurbulenceCorrelationCells = 4.0;
        public const double AijValidationBaselineSyntheticTurbulenceMaxFractionOfMean = 0.35;
        ...
        public int VtkSaveStartStep { get; set; } = 0;   // 0 preserves legacy first save at SaveInterval; >0 saves only after spin-up
        ...
        public static SimulationSettings CreateAijValidationPreflightBaseline()
        {
            return new SimulationSettings
            {
                TimeSteps = AijValidationBaselineTimeSteps,
                SaveInterval = AijValidationBaselineSaveInterval,
                EnableSyntheticTurbulentInlet = true,
                SyntheticTurbulenceIntensityScale = AijValidationBaselineSyntheticTurbulenceIntensityScale,
                SyntheticTurbulenceCorrelationCells = AijValidationBaselineSyntheticTurbulenceCorrelationCells,
                SyntheticTurbulenceModeCount = AijValidationBaselineSyntheticTurbulenceModeCount,
                SyntheticTurbulenceUpdateInterval = AijValidationBaselineSyntheticTurbulenceUpdateInterval,
                SyntheticTurbulenceMaxFractionOfMean = AijValidationBaselineSyntheticTurbulenceMaxFractionOfMean,
                SyntheticTurbulenceMinStreamwiseFraction = 0.0,
