    ...
    public class SliceVisualizationComponent : GH_Component
    {
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("Height", "H",
                "切片高度（Z坐标，单位：m）。\n" +
                "直接指定高度值，无需创建平面。\n" +
                "例如：输入 10 表示查看 Z=10m 高度的水平切片。\n" +
                "与 Slice Plane 参数互斥，优先使用 Height。",
                GH_ParamAccess.item);
            pManager[2].Optional = true;
            pManager.AddPlaneParameter("Slice Plane", "P",
                "切片平面（可选，用于非水平切片）。\n" +
                "⚠ 平面原点必须位于点云范围内！\n" +
                "当 Height 未设置时，可用此参数指定任意方向的切片平面。",
                GH_ParamAccess.item);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            double height = double.NaN; // 优先使用的高度值
            ...
            DA.GetData(3, ref slicePlane);
            DA.GetData(4, ref tolerance);
            DA.GetData(5, ref gridSize);
            ...
            {
                // 优先使用 Height 参数创建水平切片
                Point3d planeOrigin = new Point3d(bbox0.Center.X, bbox0.Center.Y, height);
                slicePlane = new Plane(planeOrigin, Vector3d.ZAxis);
                AddRuntimeMessage(GH_RuntimeMessageLevel.Remark,
                    $"使用指定高度 Z={height:F1} m 创建水平切片");
            }
            else if (!slicePlane.IsValid)
            {
                // 自动平面：使用点云中心高度的水平面
        ...
        {
            get { return IconLoader.Load("SliceVisualization.png"); }
        }
