...
def complete_manifest(required_files: list[dict]) -> dict:
    ...
    return {
        ...
        "RuntimeInletDiagnosticsGate": {"Requested": True, "Gate": "pass", "Reasons": []},
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        nested_manifest_path = case_dir / "native_fluidx3d_baseline_manifest.json"
        write(nested_manifest_path, "{}\n")
        found_from_output = module.find_run_file(output_dir, "native_fluidx3d_baseline_manifest.json")
        if found_from_output != nested_manifest_path.resolve():
            raise AssertionError(f"expected parent manifest lookup, got {found_from_output}")

        explicit_manifest_path = temp / "explicit_native_manifest.json"
        write(explicit_manifest_path, "{}\n")
        assert_gate(module, base, "pass", explicit_manifest_path)
        ...
        failed_runtime_inlet = copy.deepcopy(base)
        failed_runtime_inlet["RuntimeInletDiagnosticsGate"]["Gate"] = "fail"
        assert_gate(
            module,
            failed_runtime_inlet,
            "native_runtime_inlet_diagnostics_not_pass:fail",
            manifest_path,
        )
        ...
        allowed_run_plan_mismatch = copy.deepcopy(base)
        allowed_run_plan_mismatch["CaseToRunSourceParityGate"]["Pairs"][0]["Match"] = False
        allowed_run_plan_mismatch["CaseToRunSourceParityGate"]["Pairs"][0]["AllowedMismatch"] = True
        assert_gate(module, allowed_run_plan_mismatch, "pass", manifest_path)
