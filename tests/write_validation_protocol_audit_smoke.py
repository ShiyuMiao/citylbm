...
def create_case(root: Path) -> None:
    ...
    metadata = {
        ...
        "Uref": 4.491,
        "Zref": 0.16,
        "LbmTau": 0.500333333333333,
        "ReH": 24000.0,
        "TargetReH": 24000.0,
        "SyntheticTurbulentInletMethod": "STG-lite synthetic-eddy inlet from AF k",
...
def create_distribution_consistent_inlet_length_risk_audit(path: Path) -> None:
    audit = {
        "schema": "citylbm.inlet_source_audit.v1",
        "inlet_source_gate": "pass",
        "paper_grade_inlet_source_gate": "fail",
        "paper_grade_inlet_source_gate_reasons": ["source_missing_turbulent_length_scale_evidence"],
...
def create_passing_boundary_protocol_audit(path: Path) -> None:
    audit = {
        ...
        "floor_roughness_source_supported": True,
        "boundary_protocol_gate_reasons": [],
    }
    write(path, json.dumps(audit, indent=2))


def create_passing_coordinate_probe_audit(path: Path) -> None:
    audit = {
        ...
        "Uref": {
            "metadata_mps": 4.491,
            "metadata_zref_m": 0.16,
            "af_u_at_zref_mps": 4.491,
        },
        "OfficialProbeSummary": {
            "row_count": 186,
            "z_min": 0.02,
            "z_max": 0.02,
            "z_mismatch_count": 0,
...
def create_passing_time_averaging_evidence(path: Path) -> None:
    audit = {
...
def main() -> int:
    with tempfile.TemporaryDirectory() as raw:
        ...
        if audit["PaperGradeGate"] != "diagnostic_only":
            raise AssertionError(audit)
        if audit["PreRunGate"] != "diagnostic_only":
            raise AssertionError(audit)
        ...
        distribution_consistent_inlet = temp / "distribution_consistent_inlet_length_risk_audit.json"
        create_distribution_consistent_inlet_length_risk_audit(distribution_consistent_inlet)
        run_cmd(
            [
                sys.executable,
                str(WRITER),
                ...
                "1,0,0",
                "--inlet-source-audit",
                str(distribution_consistent_inlet),
            ]
        )
        distribution_consistent_audit = load_json(case / "validation_protocol_audit.json")
        distribution_consistent_statuses = {
            item["Key"]: item["Status"] for item in distribution_consistent_audit["Items"]
        }
        if distribution_consistent_statuses["inlet_distribution_consistency"] != "pass":
            raise AssertionError(distribution_consistent_statuses)
        if distribution_consistent_statuses["inlet_turbulence_length_scale"] != "risk":
            raise AssertionError(distribution_consistent_statuses)

        paper_grade_inlet = temp / "paper_grade_inlet_audit.json"
        paper_grade_boundary = temp / "paper_grade_boundary_audit.json"
        paper_grade_boundary_protocol = temp / "paper_grade_boundary_protocol_audit.json"
        paper_grade_coordinate = temp / "paper_grade_coordinate_probe_audit.json"
        paper_grade_time = temp / "paper_grade_time_averaging_evidence.json"
        create_paper_grade_inlet_audit(paper_grade_inlet)
        create_paper_grade_boundary_audit(paper_grade_boundary)
        create_passing_boundary_protocol_audit(paper_grade_boundary_protocol)
        create_passing_coordinate_probe_audit(paper_grade_coordinate)
        create_passing_time_averaging_evidence(paper_grade_time)
        run_cmd(
            [
                sys.executable,
                str(WRITER),
                ...
                str(paper_grade_boundary_protocol),
                "--coordinate-probe-protocol-audit",
                str(paper_grade_coordinate),
                "--time-averaging-evidence",
                str(paper_grade_time),
            ]
        )
        ...
        if paper_audit["PaperGradeGate"] != "diagnostic_only":
            raise AssertionError(paper_audit)
        if paper_audit["PreRunGate"] != "ready_for_validation_run":
            raise AssertionError(paper_audit)
        for runtime_key in ["native_fluidx3d_baseline", "systematic_bias_gate"]:
            if runtime_key not in paper_audit["FailKeys"]:
                raise AssertionError(paper_audit)
            if runtime_key in paper_audit["PreRunFailKeys"]:
                raise AssertionError(paper_audit)
        ...
        for not_expected in [
            "validation_protocol_prerun_item_fail:boundary_conditions",
            "validation_protocol_prerun_item_fail:wall_roughness_model",
        ]:
            if not_expected in reasons:
                raise AssertionError((not_expected, reasons))
        if result["ValidationProtocolAuditGate"]["Gate"] != "pass":
            raise AssertionError(result["ValidationProtocolAuditGate"])
        if result["ValidationProtocolAuditGate"]["PaperGradeGate"] != "diagnostic_only":
            raise AssertionError(result["ValidationProtocolAuditGate"])
        if result["ValidationProtocolAuditGate"]["PreRunGate"] != "ready_for_validation_run":
            raise AssertionError(result["ValidationProtocolAuditGate"])
