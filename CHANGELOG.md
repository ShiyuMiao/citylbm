...
- Packaged the current Rhino 7 / Grasshopper validation branch as the v0.4.0 GitHub release line.
- Unified plugin metadata, Yak manifest and assembly versions to `0.4.0`.
- Preserved the v0.3.0 validation-readiness work and added stricter current-run freshness checks for generated Case A canaries.
- Verified full `AF_caseA.csv` CustomTable code generation: 24 official inlet rows are emitted into `setup.cpp` with `U`, `k` and measured RMS metadata.
- Added a fresh native FluidX3D Case A 300/150 diagnostic run record with 2 complete VTK frames and 186 official probe comparisons.
